...
export default function handler(request: VercelRequest, response: VercelResponse): void {
  ...
  const password = process.env.KAIROS_OPERATOR_PASSWORD?.trim();

  if (!runtimeToken || (!passwordHash && !password)) {
    response.status(503).json({ status: "error", code: "session_unavailable", message: "Kairos operator authentication is not configured." });
    return;
  }

  if (request.method === "GET") {
    const token = readCookie(first(request.headers.cookie), SESSION_COOKIE_NAME);
  ...
  if (request.method === "POST") {
    ...
    if (!verifyOperatorPassword(accessKey, passwordHash, password)) {
