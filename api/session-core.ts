...
export function verifyOperatorPassword(supplied: string, encodedHash?: string, password?: string): boolean {
  if (encodedHash) {
    ...
    try {
      ...
      return timingSafeEqual(scryptSync(supplied, salt, PASSWORD_KEY_LENGTH), expected);
    } catch { return false; }
  }
...
export function issueOperatorSession(operatorInput: string, runtimeToken: string): { token: string; session: OperatorSession } {
  ...
  const token = `${encoded}.${sign(encoded, runtimeToken)}`;
