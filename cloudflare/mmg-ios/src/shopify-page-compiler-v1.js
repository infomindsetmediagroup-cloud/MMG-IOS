const BUILD = "shopify-page-compiler-20260712-1";

const COMPONENTS = [
  component("mmg-hero", "MMG Hero", ["homepage","landing-page","service-page","product-page"], ["eyebrow","heading","body","primaryLabel","primaryURL","secondaryLabel","secondaryURL"], "Primary message and first customer action."),
  ...
  component("cross-navigation", "Cross-Navigation Cards", ["homepage","landing-page","service-page","product-page","collection-page"], ["cards","heading"], "Prevents dead ends by linking verified next steps."),
  ...
  component("trust-standards", "Trust & Standards", ["homepage","landing-page","service-page","product-page"], ["heading","body","items"], "Explains stewardship, accuracy, approvals, and production boundaries."),
  ...
  component("kairos-guidance", "Kairos Guidance Blip", ["homepage","landing-page","service-page","product-page","portal-page"], ["message","audioURL","ctaLabel","ctaURL","frequency"], "Restrained, dismissible, captioned page guidance."),
  component("final-cta", "Final Ecosystem CTA", ["homepage","landing-page","service-page","product-page"], ["heading","body","ctaLabel","ctaURL"], "Closes the page with one verified next action.")
];
...
const BLUEPRINTS = {
  ...
  "collection-page": ["mmg-hero","knowledge-cards","cross-navigation","final-cta"],
...
async function compilePage(request) {
  ...
  selected.forEach((item,index)=>{
    ...
    template.sections[id]={ type:"mmg-page-component", settings:defaultSettings(item, title, objective), blocks:{}, block_order:[] };
  });
  const packageID=crypto.randomUUID();
  const files=[
    { filename:`templates/page.${handle}.json`, content:`${JSON.stringify(template,null,2)}\n`, purpose:"Shopify JSON page template" },
  ...
  const packageResult={
    ...
    page:{ title, handle, pageType, objective, templateSuffix:handle },
    ...
    nextAction:"Inspect Shopify staging, verify the page handle and URLs, bind source hashes, then present the executable package for approval."
  };
