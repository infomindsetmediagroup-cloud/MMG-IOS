...
export const KAIROS_LIVE_HEADER_BUILD = "kairos-live-header-navigation-20260719-2";
...
export async function handleLiveHeaderNavigationPublish(request, env) {
  ...
  const filenames = await listThemeAssets(config, auth, theme.id);
  const candidates = filenames.filter(name => /^(layout|sections|snippets|config)\/.*(header|menu|navigation|theme|settings).*(liquid|json)$/i.test(name));
  const files = await readThemeFiles(config, auth, theme.id, candidates);
  const targets = files.filter(file => file.filename.endsWith(".liquid") && (LEGACY.test(file.content || "") || /header__inline-menu|menu-drawer__navigation|section\.settings\.menu|linklists\[/i.test(file.content || "")));
  if (!targets.length) throw httpError(404, "rendered_header_source_not_found", `No rendered header Liquid source was found after enumerating ${filenames.length} live theme assets. Candidates: ${candidates.join(", ") || "none"}.`);
  ...
  return json({ status: "completed", build: KAIROS_LIVE_HEADER_BUILD, theme: { id: theme.id, name: theme.name, role: theme.role }, inventoryCount: filenames.length, scannedCandidates: candidates, patchedFiles: updates.map(file => file.filename), verification: { exactHeaderFileReadBack: true, renderedHeaderSourcePatched: updates.length > 0, canonicalNavigationEmbedded: updates.every(file => file.content.includes(KAIROS_LIVE_HEADER_BUILD)), legacyNavigationTargetFound: targets.length > 0, assetInventoryEnumerated: filenames.length > 0, nativeMenuPreserved: true } });
}
...
function injectBlock(source, block) { const stripped = stripBlock(source); const schema = stripped.lastIndexOf("{% schema %}"); return schema >= 0 ? `${stripped.slice(0, schema)}${block}\n${stripped.slice(schema)}` : `${stripped}\n${block}`; }
...
async function listThemeAssets(config, auth, themeId) { const numericId = String(themeId).split("/").pop(); const r = await fetch(`https://${config.storeDomain}/admin/api/${config.apiVersion}/themes/${numericId}/assets.json?fields=key`, { headers: { "X-Shopify-Access-Token": auth.token, Accept: "application/json" }, signal: AbortSignal.timeout(SHOPIFY_TIMEOUT_MS) }); const b = await safeJSON(r); if (!r.ok) throw httpError(r.status || 502, "theme_asset_inventory_failed", `Shopify theme asset inventory returned HTTP ${r.status}.`); return (Array.isArray(b?.assets) ? b.assets : []).map(asset => asset?.key).filter(Boolean); }
