...
const BUILD = "kairos-readiness-registry-20260714-4";
...
const DEFAULTS = {
  ...
  business: { "product-launch":70, "revenue-intelligence":45, "growth-plan":40, "offer-builder":65, "campaign-operations":35 },
...
async function applyPromotion(request, payload = {}) {
  ...
  const entry = { id:`readiness-change-${crypto.randomUUID()}`,center,capability,priorScore:currentScore,targetScore,actor,evidence,authorizationWorkflowID,appliedAt:now,build:BUILD };
      ...
      if (value?.scores) {
        ...
        for (const [center,capabilities] of Object.entries(DEFAULTS)) {
          ...
          for (const [capability,floor] of Object.entries(capabilities)) {
            ...
            if (current<floor) { value.scores[center][capability]=floor; changed=true; }
          }
        }
        if(changed){value.updatedAt=new Date().toISOString();value.build=BUILD;await caches.default.put(registryRequest(request),stored(value));}
        return value;
      }
    } catch {}
  }
  return { build:BUILD,scores:structuredClone(DEFAULTS),history:[],updatedAt:null,governance:{ authorizationRequired:true,evidenceRequired:true,noAutomaticPromotion:true,baselineMigrationFloor:true } };
}
