import kernel from "./kairos-kernel-worker-v4.js";
...
async function createApprovedStagingTheme(request, env) {
  ...
  try {
    ...
    const before = await readThemesAndContract(config, auth.accessToken);
    const mainTheme = before.themes.find(theme => theme.role === "MAIN") || null;
    const existing = before.themes.find(theme => theme.role !== "MAIN" && theme.name.toLowerCase() === "kairos staging") || null;
    ...
    const duplicate = before.duplicate;
    if (!duplicate) throw httpError(409, "theme_duplicate_unavailable", "Shopify no longer exposes themeDuplicate.");
    const argNames = new Set(duplicate.args.map(arg => arg.name));
    ...
    const payloadType = unwrapTypeName(duplicate.type);
    ...
    const selection = buildDuplicateSelection(payloadFieldNames, payloadType);

    const mutation = `
      mutation KairosCreateStaging($id: ID!, $name: String!) {
        themeDuplicate(id: $id, name: $name) ${selection}
      }
    `;
    ...
    let staging = normalizeTheme(result?.theme || result);
    const pollEvidence = [];

    for (let attempt = 1; attempt <= POLL_ATTEMPTS; attempt += 1) {
      const snapshot = await readThemes(config, auth.accessToken);
      const found = snapshot.find(theme => theme.gid === staging.gid || (theme.role !== "MAIN" && theme.name.toLowerCase() === approval.targetThemeName.toLowerCase())) || null;
      pollEvidence.push({ attempt, checkedAt: new Date().toISOString(), theme: found });
      if (found) staging = found;
      if (found && !found.processing) break;
    ...
    const verifiedStaging = afterThemes.find(theme => theme.gid === staging.gid || (theme.role !== "MAIN" && theme.name.toLowerCase() === approval.targetThemeName.toLowerCase())) || null;

    if (!verifiedStaging) throw httpError(502, "staging_theme_not_found_after_duplicate", "Shopify accepted the duplication request, but the non-live theme could not be found during verification.");
    if (verifiedStaging.role === "MAIN") throw httpError(502, "staging_theme_became_main", "Verification failed because the duplicated theme is marked MAIN.");
    ...
    if (!afterMain || afterMain.gid !== mainTheme.gid || afterMain.role !== "MAIN") {
      throw httpError(502, "main_theme_verification_failed", "The published main theme did not remain unchanged after duplication.");
    }
...
function completion(actionID, startedAt, credentialPath, config, mainTheme, stagingTheme, reusedExisting, pollEvidence) {
  return {
    ...
    rollback: {
      required: false,
      instruction: "The live theme was not modified. If this non-live copy must be removed, require a separate explicit deletion approval for this staging theme ID only.",
...
async function readThemesAndContract(config, accessToken) {
  const data = await shopifyGraphQL(config, accessToken, `
    query KairosStagingExecutionPreflight {
      ...
      themes(first: 20) { nodes { id name role processing processingFailed } }
    }
  ...
  return {
    duplicate: fields.find(field => field?.name === "themeDuplicate") || null,
    themes: Array.isArray(data?.themes?.nodes) ? data.themes.nodes.map(normalizeTheme) : [],
  };
}
...
function buildDuplicateSelection(fieldNames, payloadType) {
  if (fieldNames.has("theme")) {
    const userErrors = fieldNames.has("userErrors") ? "userErrors { field message }" : "";
    return `{ theme { id name role processing processingFailed } ${userErrors} }`;
  }
  if (/Theme$/i.test(payloadType || "")) return `{ id name role processing processingFailed }`;
  throw httpError(409, "theme_duplicate_payload_unknown", `The themeDuplicate return type ${payloadType || "unknown"} does not expose a selectable theme field.`);
}
...
async function readRequestJSON(request) {
  try { return await request.json(); } catch { throw httpError(400, "invalid_json", "The staging execution request must contain valid JSON."); }
}
...
function normalizeError(error) { return { status: Number.isInteger(error?.status) ? error.status : 500, code: typeof error?.code === "string" ? error.code : "staging_creation_failed", message: error instanceof Error ? error.message : "Staging-theme creation failed." }; }
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Runtime": BUILD, "X-Kairos-Kernel": "standalone-v5", "X-Content-Type-Options": "nosniff" } }); }
