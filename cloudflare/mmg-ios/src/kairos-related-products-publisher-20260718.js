...
export const KAIROS_RELATED_PRODUCTS_BUILD = "kairos-related-products-publisher-20260718-1";
export const RELATED_PRODUCTS_BUILD_PATH = "/api/shopify/staging/related-products/build";
export const RELATED_PRODUCTS_STAGING_CONFIRMATION = "BUILD_INTELLIGENT_RELATED_PRODUCTS_STAGING";
export const RELATED_PRODUCTS_PUBLISH_CONFIRMATION = "PUBLISH_INTELLIGENT_RELATED_PRODUCTS_LIVE";

const MANAGED_FILES = [DIGITAL_PRODUCT_TEMPLATE_FILE, SERVICE_PRODUCT_TEMPLATE_FILE, RELATED_PRODUCTS_SECTION_FILE, RELATED_PRODUCT_CARD_FILE, RELATED_PRODUCTS_CSS_FILE];
const SHOPIFY_TIMEOUT_MS = 25_000;
const READ_BACK_ATTEMPTS = 10;
const READ_BACK_DELAY_MS = 450;
const tokenCache = new Map();

export async function handleRelatedProductsBuild(request, env) {
  const url = new URL(request.url);
  if (request.method !== "POST" || url.pathname !== RELATED_PRODUCTS_BUILD_PATH) return null;
  const payload = await safeRequestJSON(request.clone());
  const mode = payload?.mode === "publish" ? "publish" : "build";
  const confirmation = mode === "publish" ? RELATED_PRODUCTS_PUBLISH_CONFIRMATION : RELATED_PRODUCTS_STAGING_CONFIRMATION;
  if (payload?.confirmation !== confirmation) throw httpError(403, "related_products_confirmation_required", `Provide the exact confirmation phrase: ${confirmation}.`);

  const productHandle = normalizeHandle(payload?.productHandle || "ai-image-mastery");
  const inspection = await inspectStagingSource(null, request, env, KAIROS_RELATED_PRODUCTS_BUILD, MANAGED_FILES);
  ...
  if (mode === "build") {
    ...
    return json({
      status: "completed",
      build: KAIROS_RELATED_PRODUCTS_BUILD,
      mode,
      summary: "Kairos installed the intelligent related-products and learning-journey system into verified Kairos Staging and confirmed exact source read-back.",
      preview: { url: `${storefrontOrigin(env)}/products/${productHandle}?preview_theme_id=${themeID(evidence.stagingTheme.gid)}`, theme: evidence.stagingTheme },
      files: candidates.map(({ content, ...file }) => file),
      safeguards: { stagingOnly: true, mainThemeMutation: false, exactReadBack: true, currentProductExcluded: true, metafieldOverridesSupported: true, collectionFallbackEnabled: true },
    });
  }
  ...
  return json({
    ...
    build: KAIROS_RELATED_PRODUCTS_BUILD,
    mode,
    summary: "Kairos promoted the verified intelligent related-products and learning-journey system to Shopify MAIN and confirmed exact source read-back.",
    production: { url: `${storefrontOrigin(env)}/products/${productHandle}`, publishedTheme: evidence.mainTheme, publishedThemeChanged: true },
    files: candidates.map(({ content, ...file }) => file),
    verification: { stagingVerifiedBeforePublish: true, publishedThemeReadBackVerified: true, exactReadBack: true },
    safeguards: { sourceBundlePromotedWithoutReconstruction: true, currentProductExcluded: true, duplicateRecommendationsExcluded: true, collectionFallbackEnabled: true },
  });
}

async function prepareCandidates() {
  const candidates = [
    { filename: DIGITAL_PRODUCT_TEMPLATE_FILE, content: RELATED_DIGITAL_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: SERVICE_PRODUCT_TEMPLATE_FILE, content: RELATED_SERVICE_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: RELATED_PRODUCTS_SECTION_FILE, content: RELATED_PRODUCTS_SECTION_SOURCE, readBackVerification: "exact-bytes" },
    { filename: RELATED_PRODUCT_CARD_FILE, content: RELATED_PRODUCT_CARD_SOURCE, readBackVerification: "exact-bytes" },
    { filename: RELATED_PRODUCTS_CSS_FILE, content: RELATED_PRODUCTS_CSS_SOURCE, readBackVerification: "exact-bytes" },
  ];
...
async function verifyThemeReadBack(env, themeGid, candidates, label) {
  ...
  throw httpError(502, "related_products_readback_mismatch", `Shopify did not expose the current ${label} related-products revision. Observed ${lastObserved.join(", ") || "no readable files"}.${detail}`);
}
...
async function readThemeFiles(env, themeGid, filenames) {
  ...
  if (errors.length) throw httpError(502, "related_products_theme_file_read_failed", errors.map(error => error.code).join(", "));
...
function normalizeHandle(value) { const handle = String(value || "").trim().toLowerCase(); if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(handle)) throw httpError(422, "invalid_product_handle", "productHandle must be a valid Shopify handle."); return handle; }
function readShopifyConfig(env) { const storeDomain = String(env.SHOPIFY_STORE_DOMAIN || "").trim().toLowerCase(); const apiVersion = String(env.SHOPIFY_API_VERSION || "2026-07").trim(); if (!/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(storeDomain)) throw httpError(503, "shopify_invalid_domain", "The Shopify store domain is invalid."); if (!/^\d{4}-\d{2}$/.test(apiVersion)) throw httpError(503, "shopify_invalid_version", "The Shopify API version is invalid."); return { storeDomain, apiVersion }; }
async function resolveAccessToken(config, env) { const clientId = String(env.SHOPIFY_CLIENT_ID || "").trim(); const clientSecret = String(env.SHOPIFY_CLIENT_SECRET || "").trim(); if (clientId && clientSecret) { const cacheKey = `${config.storeDomain}:${clientId}`; const cached = tokenCache.get(cacheKey); if (cached?.expiresAt > Date.now()) return { token: cached.token }; const response = await fetch(`https://${config.storeDomain}/admin/oauth/access_token`, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" }, body: new URLSearchParams({ grant_type: "client_credentials", client_id: clientId, client_secret: clientSecret }), signal: AbortSignal.timeout(SHOPIFY_TIMEOUT_MS) }); const body = await safeResponseJSON(response); const token = typeof body?.access_token === "string" ? body.access_token.trim() : ""; if (!response.ok || !token) throw httpError(response.status === 429 ? 429 : 401, "shopify_client_credentials_invalid", String(body?.error_description || body?.error || `Shopify token request returned HTTP ${response.status}.`).slice(0, 500)); tokenCache.set(cacheKey, { token, expiresAt: Date.now() + 55 * 60 * 1000 }); return { token }; } const token = String(env.SHOPIFY_ADMIN_ACCESS_TOKEN || "").trim(); if (!token) throw httpError(503, "shopify_not_configured", "Shopify credentials are not configured."); return { token }; }
async function shopifyGraphQL(config, auth, query, variables) { const response = await fetch(`https://${config.storeDomain}/admin/api/${config.apiVersion}/graphql.json`, { method: "POST", headers: { "X-Shopify-Access-Token": auth.token, "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ query, variables }), signal: AbortSignal.timeout(SHOPIFY_TIMEOUT_MS) }); const body = await safeResponseJSON(response); if (!response.ok) throw httpError(response.status, "shopify_graphql_http_error", body?.errors?.[0]?.message || `Shopify GraphQL returned HTTP ${response.status}.`); if (Array.isArray(body?.errors) && body.errors.length) throw httpError(422, "shopify_graphql_error", body.errors.map(error => error?.message).filter(Boolean).join("; ")); return body?.data || {}; }
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Related-Products": KAIROS_RELATED_PRODUCTS_BUILD, "X-Content-Type-Options": "nosniff" } }); }