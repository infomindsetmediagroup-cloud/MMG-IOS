export const KAIROS_FIRST_VISUAL_BATCH_BUILD = "kairos-first-visual-batch-20260728-1";

const REQUIRED_CONTENT_TYPES = Object.freeze(["manuscript", "prompt-library", "workbook"]);
const VISUAL_JOB_TYPES = new Set(["cover", "product-image"]);

export async function executeFirstVisualBatch(context = {}, input = {}) {
  requireExact(input.confirmation, "EXECUTE FIRST VISUAL BATCH");
  ...
  const contentGate = evaluateContentApprovalGate(product);
  if (!contentGate.ready) {
    throw runtimeError("CONTENT_APPROVAL_REQUIRED", "All required content assets must be approved before visual production.", 409, { blockers: contentGate.blockers });
  }

  const jobs = (Array.isArray(product.productionJobs) ? product.productionJobs : [])
    .filter((job) => VISUAL_JOB_TYPES.has(job.assetType));
  if (!jobs.length) throw runtimeError("VISUAL_JOBS_NOT_FOUND", "No visual production jobs were found.", 404);

  const receipts = [];
  for (const job of jobs) {
    if (job.authorized !== true) throw runtimeError("VISUAL_JOB_NOT_AUTHORIZED", `Visual job ${job.jobId || job.assetType} is not authorized.`, 409);
    ...
    if (!result?.asset?.checksum || !result.asset.storageRef) {
      throw runtimeError("VISUAL_ASSET_EVIDENCE_REQUIRED", `Visual job ${job.jobId || job.assetType} did not produce checksum-backed storage evidence.`, 502);
    }
  ...
  return Object.freeze({
    ...
    nextGate: "visual-review",
    automaticPublicationAllowed: false,
    build: KAIROS_FIRST_VISUAL_BATCH_BUILD,
  });
}

export function evaluateContentApprovalGate(product = {}) {
  ...
  for (const type of REQUIRED_CONTENT_TYPES) {
    ...
    else if (asset.editorialQa?.decision !== "approved") blockers.push(`${type}:not-approved`);
    else if (!asset.checksum || !asset.storageRef) blockers.push(`${type}:evidence-missing`);
  }
  return Object.freeze({ ready: blockers.length === 0, blockers: Object.freeze(blockers), requiredTypes: REQUIRED_CONTENT_TYPES });
}
