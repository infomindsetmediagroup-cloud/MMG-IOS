...
import {
  ...
  MEMBERSHIP_TEMPLATE_FILE,
  MEMBERSHIP_TEMPLATE_SOURCE,
  MEMBERSHIP_SECTION_FILE,
  MEMBERSHIP_SECTION_SOURCE,
  MEMBERSHIP_CSS_FILE,
  MEMBERSHIP_CSS_SOURCE,
} from "./kairos-canonical-homepage-source-20260718.js";

export const KAIROS_MEMBERSHIP_LANDING_BUILD = "kairos-membership-landing-publisher-20260718-1";
export const MEMBERSHIP_BUILD_PATH = "/api/shopify/staging/membership-landing/build";
export const MEMBERSHIP_STAGING_CONFIRMATION = "BUILD_CANONICAL_MMG_MEMBERSHIP_STAGING";
export const MEMBERSHIP_PUBLISH_CONFIRMATION = "PUBLISH_CANONICAL_MMG_MEMBERSHIP_LIVE";

const MANAGED_FILES = [ECOSYSTEM_NAV_FILE, MEMBERSHIP_TEMPLATE_FILE, MEMBERSHIP_SECTION_FILE, MEMBERSHIP_CSS_FILE];
const SHOPIFY_TIMEOUT_MS = 25_000;
const READ_BACK_ATTEMPTS = 10;
const READ_BACK_DELAY_MS = 450;
const tokenCache = new Map();

export async function handleMembershipLandingBuild(request, env) {
  const url = new URL(request.url);
  if (request.method !== "POST" || url.pathname !== MEMBERSHIP_BUILD_PATH) return null;
  const payload = await safeRequestJSON(request.clone());
  const mode = payload?.mode === "publish" ? "publish" : "build";
  const confirmation = mode === "publish" ? MEMBERSHIP_PUBLISH_CONFIRMATION : MEMBERSHIP_STAGING_CONFIRMATION;
  if (payload?.confirmation !== confirmation) throw httpError(403, "membership_landing_confirmation_required", `Provide the exact confirmation phrase: ${confirmation}.`);

  const inspection = await inspectStagingSource(null, request, env, KAIROS_MEMBERSHIP_LANDING_BUILD, MANAGED_FILES);
  ...
  if (mode === "build") {
    ...
    return json({
      status: "completed",
      build: KAIROS_MEMBERSHIP_LANDING_BUILD,
      mode,
      summary: "Kairos installed the canonical Membership landing page and ecosystem navigation into verified Kairos Staging and confirmed exact source read-back.",
      preview: { url: `${storefrontOrigin(env)}/pages/membership?preview_theme_id=${themeID(evidence.stagingTheme.gid)}`, theme: evidence.stagingTheme },
      files: candidates.map(({ content, ...file }) => file),
      safeguards: { stagingOnly: true, mainThemeMutation: false, exactReadBack: true },
    });
  }
  ...
  return json({
    ...
    build: KAIROS_MEMBERSHIP_LANDING_BUILD,
    mode,
    summary: "Kairos promoted the verified canonical Membership landing page and ecosystem navigation to Shopify MAIN and confirmed exact source read-back.",
    production: { url: `${storefrontOrigin(env)}/pages/membership`, publishedTheme: evidence.mainTheme, publishedThemeChanged: true },
    files: candidates.map(({ content, ...file }) => file),
    verification: { stagingVerifiedBeforePublish: true, publishedThemeReadBackVerified: true, exactReadBack: true },
    safeguards: { stagingOnly: false, mainThemeMutation: true, sourceBundlePromotedWithoutReconstruction: true },
  });
}

async function prepareCandidates() {
  const candidates = [
    { filename: ECOSYSTEM_NAV_FILE, content: ECOSYSTEM_NAV_SOURCE, readBackVerification: "exact-bytes" },
    { filename: MEMBERSHIP_TEMPLATE_FILE, content: MEMBERSHIP_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: MEMBERSHIP_SECTION_FILE, content: MEMBERSHIP_SECTION_SOURCE, readBackVerification: "exact-bytes" },
    { filename: MEMBERSHIP_CSS_FILE, content: MEMBERSHIP_CSS_SOURCE, readBackVerification: "exact-bytes" },
  ];
...
async function verifyThemeReadBack(env, themeGid, candidates, label) {
  ...
  throw httpError(502, "membership_landing_readback_mismatch", `Shopify did not expose the current ${label} Membership landing revision. Observed ${lastObserved.join(", ") || "no readable files"}.${detail}`);
}
...
async function readThemeFiles(env, themeGid, filenames) {
  ...
  if (errors.length) throw httpError(502, "membership_theme_file_read_failed", errors.map(error => error.code).join(", "));
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Membership-Landing": KAIROS_MEMBERSHIP_LANDING_BUILD, "X-Content-Type-Options": "nosniff" } }); }
