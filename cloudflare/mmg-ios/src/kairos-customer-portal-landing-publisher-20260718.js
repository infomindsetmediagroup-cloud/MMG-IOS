...
export const KAIROS_CUSTOMER_PORTAL_LANDING_BUILD = "kairos-customer-portal-landing-publisher-20260718-1";
export const CUSTOMER_PORTAL_BUILD_PATH = "/api/shopify/staging/customer-portal-landing/build";
export const CUSTOMER_PORTAL_STAGING_CONFIRMATION = "BUILD_CANONICAL_MMG_CUSTOMER_PORTAL_STAGING";
export const CUSTOMER_PORTAL_PUBLISH_CONFIRMATION = "PUBLISH_CANONICAL_MMG_CUSTOMER_PORTAL_LIVE";

const MANAGED_FILES = [ECOSYSTEM_NAV_FILE, CUSTOMER_PORTAL_TEMPLATE_FILE, CUSTOMER_PORTAL_SECTION_FILE, CUSTOMER_PORTAL_CSS_FILE];
const SHOPIFY_TIMEOUT_MS = 25_000;
const READ_BACK_ATTEMPTS = 10;
const READ_BACK_DELAY_MS = 450;
const tokenCache = new Map();

export async function handleCustomerPortalLandingBuild(request, env) {
  const url = new URL(request.url);
  if (request.method !== "POST" || url.pathname !== CUSTOMER_PORTAL_BUILD_PATH) return null;
  const payload = await safeRequestJSON(request.clone());
  const mode = payload?.mode === "publish" ? "publish" : "build";
  const confirmation = mode === "publish" ? CUSTOMER_PORTAL_PUBLISH_CONFIRMATION : CUSTOMER_PORTAL_STAGING_CONFIRMATION;
  if (payload?.confirmation !== confirmation) throw httpError(403, "customer_portal_landing_confirmation_required", `Provide the exact confirmation phrase: ${confirmation}.`);

  const inspection = await inspectStagingSource(null, request, env, KAIROS_CUSTOMER_PORTAL_LANDING_BUILD, MANAGED_FILES);
  ...
  if (mode === "build") {
    ...
    return json({
      status: "completed",
      build: KAIROS_CUSTOMER_PORTAL_LANDING_BUILD,
      mode,
      summary: "Kairos installed the canonical Customer Portal landing page and ecosystem navigation into verified Kairos Staging and confirmed exact source read-back.",
      preview: { url: `${storefrontOrigin(env)}/pages/customer-portal?preview_theme_id=${themeID(evidence.stagingTheme.gid)}`, theme: evidence.stagingTheme },
      files: candidates.map(({ content, ...file }) => file),
      safeguards: { stagingOnly: true, mainThemeMutation: false, exactReadBack: true, authenticatedWorkspaceEntryOnly: true },
    });
  }
  ...
  return json({
    ...
    build: KAIROS_CUSTOMER_PORTAL_LANDING_BUILD,
    mode,
    summary: "Kairos promoted the verified canonical Customer Portal landing page and ecosystem navigation to Shopify MAIN and confirmed exact source read-back.",
    production: { url: `${storefrontOrigin(env)}/pages/customer-portal`, publishedTheme: evidence.mainTheme, publishedThemeChanged: true },
    files: candidates.map(({ content, ...file }) => file),
    verification: { stagingVerifiedBeforePublish: true, publishedThemeReadBackVerified: true, exactReadBack: true },
    safeguards: { stagingOnly: false, mainThemeMutation: true, sourceBundlePromotedWithoutReconstruction: true, authenticatedWorkspaceEntryOnly: true },
  });
}
...
async function prepareCandidates() {
  const candidates = [
    ...
    { filename: CUSTOMER_PORTAL_TEMPLATE_FILE, content: CUSTOMER_PORTAL_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: CUSTOMER_PORTAL_SECTION_FILE, content: CUSTOMER_PORTAL_SECTION_SOURCE, readBackVerification: "exact-bytes" },
    { filename: CUSTOMER_PORTAL_CSS_FILE, content: CUSTOMER_PORTAL_CSS_SOURCE, readBackVerification: "exact-bytes" },
  ];
...
async function verifyThemeReadBack(env, themeGid, candidates, label) {
  ...
  throw httpError(502, "customer_portal_landing_readback_mismatch", `Shopify did not expose the current ${label} Customer Portal landing revision. Observed ${lastObserved.join(", ") || "no readable files"}.${detail}`);
}
...
async function readThemeFiles(env, themeGid, filenames) {
  ...
  if (errors.length) throw httpError(502, "customer_portal_theme_file_read_failed", errors.map(error => error.code).join(", "));
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Customer-Portal-Landing": KAIROS_CUSTOMER_PORTAL_LANDING_BUILD, "X-Content-Type-Options": "nosniff" } }); }