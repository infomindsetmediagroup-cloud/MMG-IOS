import runtime from "./kairos-command-hub-v27.js";
import { applyCompactPatch, httpError, inspectStagingSource, parseShopifyJson, safeJSON, semanticHash, writeThemeFile } from "./kairos-compact-homepage-utils-v1.js";
import { buildDeterministicHomepagePackage } from "./kairos-deterministic-homepage-v1.js";

const BUILD = "kairos-command-hub-20260712-29";
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (planMatch && request.method === "GET") return readJob(request, "plan", planMatch[1]);
    if (url.pathname === "/api/shopify/staging/execute/jobs" && request.method === "POST") return executePlan(request, env);
    const executionMatch = url.pathname.match(/^\/api\/shopify\/staging\/execute\/jobs\/([a-f0-9-]+)$/i);
    if (executionMatch && request.method === "GET") return readJob(request, "execution", executionMatch[1]);

    const response = await runtime.fetch(request, env, ctx);
    if (url.pathname === "/api/health" || url.pathname === "/api/capabilities") {
      const body = await safeJSON(response.clone());
      body.build = BUILD;
      body.kernel = "command-hub-v29";
      body.capabilities = {
        ...(body.capabilities || {}),
...
async function createPlan(request, env) {
  try {
    ...
    const sourceBody = await inspectStagingSource(runtime, request, env, BUILD);
...
async function executePlan(request, env) {
  ...
  try {
    ...
    const verifyBody = await inspectStagingSource(runtime, request, env, BUILD);
...
async function readJob(request, type, jobID) {
  ...
  if (!response) return json({ jobID, status: "not-found", error: { message: `The website ${type} job was not found or expired.` } }, 404);
...
function jobRequest(request, type, jobID) {
  return new Request(new URL(`/_kairos/v29-${type}-jobs/${jobID}`, request.url).toString(), { method: "GET" });
}
...
function failure(summary, error) {
  ...
  const normalized = { status, code: typeof error?.code === "string" ? error.code : "deterministic_homepage_failed", message: error instanceof Error ? error.message : "Deterministic homepage operation failed." };
...
function retag(response) {
  ...
  headers.set("X-Kairos-Kernel", "command-hub-v29");
  return new Response(response.body, { status: response.status, statusText: response.statusText, headers });
}

function json(value, status = 200) {
  return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Runtime": BUILD, "X-Kairos-Kernel": "command-hub-v29", "X-Content-Type-Options": "nosniff" } });
}
