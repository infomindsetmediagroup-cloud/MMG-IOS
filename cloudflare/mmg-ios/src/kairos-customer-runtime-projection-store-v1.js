...
import { verifyShopifyCustomerAccountSession, KAIROS_SHOPIFY_CUSTOMER_ACCOUNT_AUTH_BUILD } from "./shopify/kairos-shopify-customer-account-auth-v1.js";

export const KAIROS_CUSTOMER_RUNTIME_PROJECTION_STORE_BUILD = "kairos-customer-runtime-projection-store-20260807-4-shopify-session-jwt";
...
export async function handleKairosCustomerRuntimeProjectionAPI(request, env) {
  ...
  const auth = await verifyShopifyCustomerAccountSession(request, env);
  if (!auth.ok) {
    if (auth.status === 503) {
      return json({
        success: false,
  ...
  const identity = {
    ...
    kairosCustomerId: mapKairosCustomerId(auth.shopifyCustomerId, env),
  };
...
function builds() {
  return {
    ...
    customerAccountAuth: KAIROS_SHOPIFY_CUSTOMER_ACCOUNT_AUTH_BUILD,
  };
}
...
function method(allowed) {
  return json({ success: false, error: { code: "METHOD_NOT_ALLOWED", message: `Use ${allowed}.` } }, 405, { Allow: allowed });
}
...
function preflight() {
  return new Response(null, {
    ...
    headers: {
      ...
      "X-Kairos-Customer-Account-Auth": KAIROS_SHOPIFY_CUSTOMER_ACCOUNT_AUTH_BUILD,
    },
  });
}
