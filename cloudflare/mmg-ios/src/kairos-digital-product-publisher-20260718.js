...
export const KAIROS_DIGITAL_PRODUCT_BUILD = "kairos-digital-product-publisher-20260718-1";
export const DIGITAL_PRODUCT_BUILD_PATH = "/api/shopify/staging/digital-product/build";
export const DIGITAL_PRODUCT_STAGING_CONFIRMATION = "BUILD_CANONICAL_MMG_DIGITAL_PRODUCT_STAGING";
export const DIGITAL_PRODUCT_PUBLISH_CONFIRMATION = "PUBLISH_CANONICAL_MMG_DIGITAL_PRODUCT_LIVE";

const MANAGED_FILES = [DIGITAL_PRODUCT_TEMPLATE_FILE, DIGITAL_PRODUCT_SECTION_FILE, DIGITAL_PRODUCT_CSS_FILE];
const SHOPIFY_TIMEOUT_MS = 25_000;
const READ_BACK_ATTEMPTS = 10;
const READ_BACK_DELAY_MS = 450;
const tokenCache = new Map();

export async function handleDigitalProductBuild(request, env) {
  const url = new URL(request.url);
  if (request.method !== "POST" || url.pathname !== DIGITAL_PRODUCT_BUILD_PATH) return null;
  const payload = await safeRequestJSON(request.clone());
  const mode = payload?.mode === "publish" ? "publish" : "build";
  const confirmation = mode === "publish" ? DIGITAL_PRODUCT_PUBLISH_CONFIRMATION : DIGITAL_PRODUCT_STAGING_CONFIRMATION;
  if (payload?.confirmation !== confirmation) throw httpError(403, "digital_product_confirmation_required", `Provide the exact confirmation phrase: ${confirmation}.`);

  const productHandle = normalizeHandle(payload?.productHandle || "ai-image-mastery");
  const inspection = await inspectStagingSource(null, request, env, KAIROS_DIGITAL_PRODUCT_BUILD, MANAGED_FILES);
  ...
  if (mode === "build") {
    ...
    return json({
      status: "completed",
      build: KAIROS_DIGITAL_PRODUCT_BUILD,
      mode,
      summary: "Kairos installed the canonical single-variant digital-product system into verified Kairos Staging and confirmed exact source read-back.",
      preview: { url: `${storefrontOrigin(env)}/products/${productHandle}?preview_theme_id=${themeID(evidence.stagingTheme.gid)}`, theme: evidence.stagingTheme, templateSuffix: "mmg-digital" },
      files: candidates.map(({ content, ...file }) => file),
      safeguards: { stagingOnly: true, mainThemeMutation: false, exactReadBack: true, productDataDriven: true, singleVariantArchitectureVerified: true },
    });
  }
  ...
  return json({
    ...
    build: KAIROS_DIGITAL_PRODUCT_BUILD,
    mode,
    summary: "Kairos promoted the verified canonical single-variant digital-product system to Shopify MAIN and confirmed exact source read-back.",
    production: { url: `${storefrontOrigin(env)}/products/${productHandle}`, publishedTheme: evidence.mainTheme, publishedThemeChanged: true, templateSuffix: "mmg-digital" },
    files: candidates.map(({ content, ...file }) => file),
    verification: { stagingVerifiedBeforePublish: true, publishedThemeReadBackVerified: true, exactReadBack: true },
    safeguards: { stagingOnly: false, mainThemeMutation: true, sourceBundlePromotedWithoutReconstruction: true, singleVariantArchitectureVerified: true },
  });
}

async function prepareCandidates() {
  const candidates = [
    { filename: DIGITAL_PRODUCT_TEMPLATE_FILE, content: DIGITAL_PRODUCT_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: DIGITAL_PRODUCT_SECTION_FILE, content: DIGITAL_PRODUCT_SECTION_SOURCE, readBackVerification: "exact-bytes" },
    { filename: DIGITAL_PRODUCT_CSS_FILE, content: DIGITAL_PRODUCT_CSS_SOURCE, readBackVerification: "exact-bytes" },
  ];
...
async function verifyThemeReadBack(env, themeGid, candidates, label) {
  ...
  throw httpError(502, "digital_product_readback_mismatch", `Shopify did not expose the current ${label} digital-product revision. Observed ${lastObserved.join(", ") || "no readable files"}.${detail}`);
}
...
async function readThemeFiles(env, themeGid, filenames) {
  ...
  if (errors.length) throw httpError(502, "digital_product_theme_file_read_failed", errors.map(error => error.code).join(", "));
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Digital-Product": KAIROS_DIGITAL_PRODUCT_BUILD, "X-Content-Type-Options": "nosniff" } }); }
