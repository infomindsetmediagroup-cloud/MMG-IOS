const BUILD = "shopify-visual-verification-20260712-1";
const REVIEW_TTL_SECONDS = 60 * 60 * 24;

export async function handleVisualVerificationRequest(request, env) {
  const url = new URL(request.url);

  if (url.pathname === "/api/shopify/staging/visual-verification" && request.method === "POST") {
    return createVerification(request, env);
  }

  if (url.pathname === "/api/shopify/staging/visual-approval" && request.method === "POST") {
    return approveVerification(request);
  }
...
async function createVerification(request, env) {
  ...
  const execution = body?.execution || body?.result?.execution || {};
  ...
  const store = String(env.SHOPIFY_STORE_DOMAIN || body?.shopifyStore || "07kd8e-qw.myshopify.com").trim().toLowerCase();
  const path = normalizePath(body?.path || pathForRequestType(body?.requestType));
  const previewURL = `https://${store}${path}${path.includes("?") ? "&" : "?"}preview_theme_id=${encodeURIComponent(themeID)}`;
  ...
  const record = {
    ...
    requiredVisualChecks: [
      ...
      "The live published theme remains unchanged."
    ],
...
async function approveVerification(request) {
  ...
  const updated = {
    ...
    executiveDecision: {
      decision,
      actor: String(body?.actor || "Executive"),
      decidedAt: new Date().toISOString(),
      notes: String(body?.notes || "").slice(0, 2000)
    },
    publicationAuthorized: false,
    nextAction: decision === "approved"
      ? "Open Release Control to approve staging-to-live publication."
      : "Return the website job to revision; do not publish this staging result."
  };
  await caches.default.put(reviewRequest(request, reviewID), new Response(JSON.stringify(updated), {
    headers: { "Content-Type": "application/json", "Cache-Control": `max-age=${REVIEW_TTL_SECONDS}` }
  }));
  return json(updated);
}
...
async function probePreview(previewURL) {
  ...
  try {
    const response = await fetch(previewURL, {
      redirect: "follow",
      headers: { "User-Agent": "Kairos-Visual-Verification/1.0", "Accept": "text/html" }
    });
    ...
    return {
      checks: [check("preview_reachable", false, error instanceof Error ? error.message : "Preview could not be reached")],
      evidence: { finalURL: previewURL, httpStatus: 0, latencyMs: Date.now() - started, error: error instanceof Error ? error.message : "Preview probe failed" }
    };
  }
}
