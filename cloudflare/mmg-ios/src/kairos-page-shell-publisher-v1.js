...
const PAGE_RULES = {
  "/pages/free-creator-toolkit": {
    ...
    nativeTitle: "Free Creator Toolkit",
    heroTitle: "The Free Creator Toolkit.",
    normalizeFooter: "toolkit-mini-navigation",
  },
  "/pages/publishing-services": {
    key: "publishing-services",
    nativeTitle: "MMG Publishing Services™",
    heroTitle: "Turn your knowledge into a published book.",
...
.mmg-native-page-title-hidden,
.mmg-page-directory-hidden,
.mmg-page-mini-navigation-hidden{display:none!important}
html[data-mmg-page-shell-page="free-creator-toolkit"] main,
html[data-mmg-page-shell-page="publishing-services"] main{overflow-x:clip}
html[data-mmg-page-shell-page] #publishing-process{scroll-margin-top:10rem}
`;
...
export async function handlePageShellPublish(request, env) {
  ...
  if (payload?.confirmation !== PAGE_SHELL_CONFIRMATION) {
    throw httpError(
      403,
      "page_shell_confirmation_required",
      `Provide the exact confirmation phrase: ${PAGE_SHELL_CONFIRMATION}.`,
    );
  }
  ...
  const beforeFiles = await readThemeFiles(config, auth, mainTheme.id, [
    LAYOUT_FILE,
    CSS_FILE,
    JS_FILE,
  ]);
  ...
  if (!layoutBefore) {
    throw httpError(
      409,
      "page_shell_layout_unavailable",
      `${LAYOUT_FILE} was not readable from the published Shopify MAIN theme.`,
    );
  }
  ...
  const updates = [
    {
      ...
      content: injectPageShellAssets(stripManagedBlock(layoutBefore)),
    },
  ...
  return json({
    ...
    summary:
      "Published the shared MMG page-shell reconciler for canonical titles, publishing-process links, and page-specific footer normalization.",
    pages: Object.entries(PAGE_RULES).map(([path, rule]) => ({ path, ...rule })),
    theme: summarizeTheme(mainTheme),
    files: await Promise.all(
      updates.map(async (file) => ({
...
function buildPageShellRuntime() {
  ...
  function markHidden(element, className) {
    if (!element || element.classList.contains(className)) return false;
    element.classList.add(className);
    element.setAttribute("aria-hidden", "true");
    element.dataset.mmgPageShell = BUILD;
    return true;
  }

  function hideDuplicateNativeTitle(main, rule) {
    const headings = Array.from(main.querySelectorAll("h1"));
    const nativeTitle = normalize(rule.nativeTitle).toLowerCase();
    const heroTitle = normalize(rule.heroTitle).toLowerCase();

    let candidate = headings.find(function(heading) {
      const text = normalize(heading.textContent).toLowerCase();
      return text === nativeTitle && text !== heroTitle;
    });

    if (!candidate) {
      const visible = headings.filter(isVisible);
      const hero = visible.find(function(heading) {
        return normalize(heading.textContent).toLowerCase() === heroTitle;
      });
      if (visible.length > 1 && hero) {
        candidate = visible.find(function(heading) {
          return heading !== hero &&
            (heading.matches(".main-page-title,.page-title,[class*='page-title']") ||
             heading.compareDocumentPosition(hero) & Node.DOCUMENT_POSITION_FOLLOWING);
        });
      }
    }

    return markHidden(candidate, "mmg-native-page-title-hidden");
  }
  ...
  function ensurePublishingProcessTarget(main) {
    ...
    const heading = Array.from(main.querySelectorAll("h2,h3,[role='heading']")).find(function(element) {
      ...
      return text === "the publishing workflow is designed to be clear." ||
        text === "our process";
    });
    if (!heading) return null;

    target = heading.closest("section") || heading.closest("div") || heading;
  ...
  function repairProjectGuideLinks(main) {
    ...
    Array.from(main.querySelectorAll("a[href]")).forEach(function(anchor) {
      ...
      if (
        ...
        parsed.pathname.replace(/\/+$/, "") !== "/pages/project-guide"
      ) {
        return;
      }
  ...
  function hidePublishingDirectory(main) {
    ...
    let candidate = heading.closest("section");
    if (!candidate) {
      candidate = heading.parentElement;
      while (
        candidate &&
        candidate !== main &&
        candidate.querySelectorAll("a").length < 6
      ) {
        candidate = candidate.parentElement;
      }
    }
    if (!candidate || candidate === main) return false;
    return markHidden(candidate, "mmg-page-directory-hidden");
  }
  ...
  function hideToolkitMiniNavigation(main) {
    ...
    const home = Array.from(main.querySelectorAll("a")).find(function(anchor) {
      return normalize(anchor.textContent) === "MMG Home";
    });
    if (!home) return false;

    let best = null;
    let candidate = home.parentElement;
    for (let depth = 0; candidate && candidate !== main && depth < 8; depth += 1) {
      const links = Array.from(candidate.querySelectorAll("a"));
      const labels = links.map(function(anchor) { return normalize(anchor.textContent); });
      const matches = labels.filter(function(label) { return known.has(label); }).length;
      const containsPageHero = Array.from(candidate.querySelectorAll("h1")).some(function(heading) {
        return normalize(heading.textContent) === "The Free Creator Toolkit.";
      });

      if (!containsPageHero && matches >= 4 && links.length <= 12) {
        best = candidate;
        break;
      }
      candidate = candidate.parentElement;
    ...
    if (!best) {
      best = home.closest("nav") || home.closest("section");
    }
    if (!best || best === main) return false;
    return markHidden(best, "mmg-page-mini-navigation-hidden");
  }
  ...
  function install() {
    ...
    document.documentElement.dataset.mmgPageShellPage = rule.key;

    const result = {
      titleHidden: hideDuplicateNativeTitle(main, rule),
      linksRepaired: rule.repairProjectGuideLinks ? repairProjectGuideLinks(main) : 0,
      directoryHidden:
        rule.normalizeFooter === "publishing-directory"
          ? hidePublishingDirectory(main)
          : false,
      miniNavigationHidden:
        rule.normalizeFooter === "toolkit-mini-navigation"
          ? hideToolkitMiniNavigation(main)
          : false,
    };

    document.documentElement.dataset.mmgPageShellReady = "true";
    window.dispatchEvent(new CustomEvent("mmg:page-shell-ready", {
      detail: { build: BUILD, page: rule.key, result },
    }));
    return true;
  }
...
${MARKER_END}`;
  ...
  throw httpError(
    409,
    "page_shell_layout_invalid",
    `${LAYOUT_FILE} contains neither </head> nor </body>.`,
  );
}
...
async function getMainTheme(config, auth) {
  const data = await shopifyGraphQL(
    config,
    auth,
    `query KairosPageShellMainTheme {
      themes(first: 20) {
  ...
  const main = (data?.themes?.nodes || []).find(
    (theme) => String(theme?.role || "").toUpperCase() === "MAIN",
  );
  if (!main?.id) {
    throw httpError(
      409,
      "page_shell_main_theme_not_found",
      "The published Shopify MAIN theme could not be identified.",
    );
  }
  if (main.processing || main.processingFailed) {
    throw httpError(
      409,
      "page_shell_main_theme_not_ready",
      "The published Shopify MAIN theme is processing or failed processing.",
    );
  }
  return main;
}
...
async function readThemeFiles(config, auth, themeId, filenames) {
  ...
  const errors = (connection?.userErrors || []).filter(
    (item) => item?.code && item.code !== "NOT_FOUND",
  );
  if (errors.length) {
    throw httpError(
      502,
      "page_shell_file_read_failed",
...
async function verifyReadBack(config, auth, themeId, candidates) {
  for (let attempt = 1; attempt <= READ_BACK_ATTEMPTS; attempt += 1) {
    const actualFiles = await readThemeFiles(
      config,
      auth,
      themeId,
      candidates.map((file) => file.filename),
    );
    ...
    for (const candidate of candidates) {
      ...
      if (
        !actual ||
        actual.content !== candidate.content ||
  ...
  throw httpError(
    502,
    "page_shell_theme_readback_failed",
    "Shopify did not preserve the exact MMG page-shell files.",
  );
}
...
async function restoreFiles(env, themeId, candidates, beforeMap) {
  const restore = candidates
    ...
    .map((file) => ({
      filename: file.filename,
      content: beforeMap.get(file.filename).content,
    }));
...
function readShopifyConfig(env) {
  const storeDomain = String(env.SHOPIFY_STORE_DOMAIN || "")
    .trim()
    .toLowerCase();
  ...
  if (!/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(storeDomain)) {
    throw httpError(
      503,
      "page_shell_shopify_invalid_domain",
  ...
  if (!/^\d{4}-\d{2}$/.test(apiVersion)) {
    throw httpError(
      503,
      "page_shell_shopify_invalid_version",
      "The Shopify API version is invalid.",
    );
  }
...
async function resolveAccessToken(config, env) {
  ...
  if (clientId && clientSecret) {
    ...
    if (!response.ok || !token) {
      throw httpError(
        401,
        "page_shell_shopify_client_credentials_invalid",
  ...
  if (!token) {
    throw httpError(
      503,
      "page_shell_shopify_not_configured",
...
async function shopifyGraphQL(config, auth, query, variables) {
  ...
  if (!response.ok) {
    throw httpError(
      response.status,
      "page_shell_shopify_graphql_http_error",
  ...
  if (body?.errors?.length) {
    throw httpError(
      422,
      "page_shell_shopify_graphql_error",
      body.errors
