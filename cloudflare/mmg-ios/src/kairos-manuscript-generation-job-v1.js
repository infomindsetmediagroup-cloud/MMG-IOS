export const KAIROS_MANUSCRIPT_GENERATION_BUILD = "kairos-manuscript-generation-job-20260725-2-openai";
...
async function startJob(state,env,projectId) {
  ...
  if(provider.provider==="deterministic") throw fail(503,"generation_provider_required","Backend manuscript generation requires the configured OpenAI or Ollama-compatible provider. Kairos will not fall back to the iPhone runtime.");
  if(!provider.endpoint) throw fail(503,"generation_endpoint_required","KAIROS_MODEL_ENDPOINT is not configured.");
  if(provider.provider==="openai"&&!provider.token) throw fail(503,"generation_api_key_required","OPENAI_API_KEY is not configured in the Kairos Worker.");
...
async function runOneStep(state,env,projectId,job) {
  ...
  try {
    ...
    await state.storage.put(jobKey(projectId),{...running,step:job.step+1,generatedWords:job.generatedWords+words,totalWords:job.totalWords+words,attempts:0,phase:"Section stored and verified",updatedAt:new Date().toISOString()});
  } catch(error) {
...
function providerConfig(env){const provider=String(env?.KAIROS_MODEL_PROVIDER||"deterministic").toLowerCase();return{provider,endpoint:String(env?.KAIROS_MODEL_ENDPOINT||"").replace(/\/$/,""),model:String(env?.KAIROS_MODEL_NAME||env?.KAIROS_OPENAI_MODEL||"gpt-5-mini"),token:String(env?.KAIROS_MODEL_AUTH_TOKEN||env?.OPENAI_API_KEY||"")};}
async function generate(config,prompt){if(!config.endpoint)throw new Error("KAIROS_MODEL_ENDPOINT is not configured.");const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),120000);try{if(config.provider==="ollama"){const response=await fetch(`${config.endpoint}/api/generate`,{method:"POST",headers:headers(config),body:JSON.stringify({model:config.model,stream:false,prompt,options:{temperature:0.35}}),signal:controller.signal});const body=await requireJSON(response);return body.response||"";}if(config.provider==="openai"){const response=await fetch(`${config.endpoint}/v1/responses`,{method:"POST",headers:headers(config),body:JSON.stringify({model:config.model,instructions:"You are Kairos, the source-grounded editorial engine for Mindset Media Group. Return only polished customer-facing manuscript content. Never invent facts, citations, statistics, guarantees, private URLs, people, products, or events.",input:prompt,max_output_tokens:5000}),signal:controller.signal});const body=await requireJSON(response);return extractOpenAIText(body);}if(config.provider==="openai-compatible"){const response=await fetch(`${config.endpoint}/v1/chat/completions`,{method:"POST",headers:headers(config),body:JSON.stringify({model:config.model,temperature:0.35,messages:[{role:"system",content:"You are Kairos, the source-grounded editorial engine for Mindset Media Group. Return only polished customer-facing manuscript content. Never invent facts, citations, statistics, guarantees, private URLs, people, products, or events."},{role:"user",content:prompt}]}),signal:controller.signal});const body=await requireJSON(response);return body?.choices?.[0]?.message?.content||"";}throw new Error(`Unsupported backend model provider: ${config.provider}`);}finally{clearTimeout(timeout);}}
function extractOpenAIText(body){if(typeof body?.output_text==="string")return body.output_text;const parts=[];for(const item of body?.output||[]){for(const content of item?.content||[]){if(content?.type==="output_text"&&content?.text)parts.push(content.text);}}return parts.join("\n\n");}
...
async function requireJSON(response){const text=await response.text();if(!response.ok){let detail="";try{detail=JSON.parse(text)?.error?.message||"";}catch{}throw new Error(`Backend model returned HTTP ${response.status}${detail?`: ${detail}`:"."}`);}try{return JSON.parse(text);}catch{throw new Error("Backend model returned unreadable output.");}}
