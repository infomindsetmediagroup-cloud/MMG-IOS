...
export const KAIROS_KNOWLEDGE_LANDING_BUILD = "kairos-knowledge-landing-publisher-20260718-1";
export const KNOWLEDGE_BUILD_PATH = "/api/shopify/staging/knowledge-landing/build";
export const KNOWLEDGE_STAGING_CONFIRMATION = "BUILD_CANONICAL_MMG_KNOWLEDGE_STAGING";
export const KNOWLEDGE_PUBLISH_CONFIRMATION = "PUBLISH_CANONICAL_MMG_KNOWLEDGE_LIVE";

const MANAGED_FILES = [ECOSYSTEM_NAV_FILE, KNOWLEDGE_TEMPLATE_FILE, KNOWLEDGE_SECTION_FILE, KNOWLEDGE_CSS_FILE];
const SHOPIFY_TIMEOUT_MS = 25_000;
const READ_BACK_ATTEMPTS = 10;
const READ_BACK_DELAY_MS = 450;
const tokenCache = new Map();

export async function handleKnowledgeLandingBuild(request, env) {
  const url = new URL(request.url);
  if (request.method !== "POST" || url.pathname !== KNOWLEDGE_BUILD_PATH) return null;
  const payload = await safeRequestJSON(request.clone());
  const mode = payload?.mode === "publish" ? "publish" : "build";
  const confirmation = mode === "publish" ? KNOWLEDGE_PUBLISH_CONFIRMATION : KNOWLEDGE_STAGING_CONFIRMATION;
  if (payload?.confirmation !== confirmation) throw httpError(403, "knowledge_landing_confirmation_required", `Provide the exact confirmation phrase: ${confirmation}.`);

  const inspection = await inspectStagingSource(null, request, env, KAIROS_KNOWLEDGE_LANDING_BUILD, MANAGED_FILES);
  ...
  if (mode === "build") {
    ...
    return json({
      status: "completed",
      build: KAIROS_KNOWLEDGE_LANDING_BUILD,
      mode,
      summary: "Kairos installed the canonical Knowledge landing page and ecosystem navigation into verified Kairos Staging and confirmed exact source read-back.",
      preview: { url: `${storefrontOrigin(env)}/pages/knowledge-library?preview_theme_id=${themeID(evidence.stagingTheme.gid)}`, theme: evidence.stagingTheme },
  ...
  return json({
    ...
    build: KAIROS_KNOWLEDGE_LANDING_BUILD,
    mode,
    summary: "Kairos promoted the verified canonical Knowledge landing page and ecosystem navigation to Shopify MAIN and confirmed exact source read-back.",
    production: { url: `${storefrontOrigin(env)}/pages/knowledge-library`, publishedTheme: evidence.mainTheme, publishedThemeChanged: true },
    files: candidates.map(({ content, ...file }) => file),
...
async function prepareCandidates() {
  const candidates = [
    ...
    { filename: KNOWLEDGE_TEMPLATE_FILE, content: KNOWLEDGE_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: KNOWLEDGE_SECTION_FILE, content: KNOWLEDGE_SECTION_SOURCE, readBackVerification: "exact-bytes" },
    { filename: KNOWLEDGE_CSS_FILE, content: KNOWLEDGE_CSS_SOURCE, readBackVerification: "exact-bytes" },
  ];
...
async function verifyThemeReadBack(env, themeGid, candidates, label) {
  ...
  throw httpError(502, "knowledge_landing_readback_mismatch", `Shopify did not expose the current ${label} Knowledge landing revision. Observed ${lastObserved.join(", ") || "no readable files"}.${detail}`);
}
...
async function readThemeFiles(env, themeGid, filenames) {
  ...
  if (errors.length) throw httpError(502, "knowledge_theme_file_read_failed", errors.map(error => error.code).join(", "));
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Knowledge-Landing": KAIROS_KNOWLEDGE_LANDING_BUILD, "X-Content-Type-Options": "nosniff" } }); }