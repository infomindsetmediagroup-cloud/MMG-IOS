...
export const KAIROS_REVENUE_PRODUCT_STORE_BUILD = "kairos-revenue-product-store-20260727-4";
...
const ACTION = /^\/api\/kairos\/revenue\/products\/([^/]+)\/(plan-jobs|approve|authorize-job|execute-job|execute-next|execution-queue|complete-job|register-asset|shopify-handoff|export)\/?$/i;
...
async function executeAction(env, revenueProductId, input, executeNext) {
  try {
    ...
    const persisted = await forward(env, {
      operation: "record-stored-execution",
      revenueProductId,
      input: {
        execution: summarizeExecution(execution),
        receipt,
        asset: { ...storedAsset, operatorIdentityHash: input.operatorIdentityHash },
        jobId: job.jobId,
...
function recordStoredExecution(record, input) {
  const registered = registerKairosRevenueAsset(record, input.asset || {});
  const completed = completeKairosRevenueJob(registered, { jobId: input.jobId, completedAt: input.completedAt, outputAssetIds: [input.asset?.assetId] });
  const receipts = [...(Array.isArray(record.executionReceipts) ? record.executionReceipts : []), input.receipt || {}].slice(-100);
  return rebuild({ ...completed, executionReceipts: Object.freeze(receipts), lastExecution: Object.freeze(input.execution || {}), updatedAt: now() });
}
...
function builds() { return { engine: KAIROS_REVENUE_ENGINE_BUILD, jobs: KAIROS_REVENUE_PRODUCTION_JOBS_BUILD, assets: KAIROS_REVENUE_ASSET_REGISTRATION_BUILD, assetStorage: KAIROS_REVENUE_ASSET_STORAGE_BUILD, sequencer: KAIROS_REVENUE_JOB_SEQUENCER_BUILD, handoff: KAIROS_SHOPIFY_PUBLICATION_HANDOFF_BUILD, executor: KAIROS_OPENAI_REVENUE_EXECUTOR_BUILD, receipts: KAIROS_REVENUE_EXECUTION_RECEIPT_BUILD, store: KAIROS_REVENUE_PRODUCT_STORE_BUILD }; }
