...
export const KAIROS_REVENUE_PRODUCT_STORE_BUILD = "kairos-revenue-product-store-20260727-3";
...
const ACTION = /^\/api\/kairos\/revenue\/products\/([^/]+)\/(plan-jobs|approve|authorize-job|execute-job|complete-job|register-asset|shopify-handoff|export)\/?$/i;
...
export async function handleKairosRevenueProductAPI(request, env) {
  ...
  if (action) {
    ...
    const input = enrich(await body(request), identity, env);
    const revenueProductId = clean(action[1], 180);
    if (action[2] === "execute-job") return executeAction(env, revenueProductId, input);
    return forward(env, { operation: action[2], revenueProductId, input });
  }
...
export async function handleKairosRevenueProductObjectRequest(state, request) {
  ...
  if (input.operation === "record-execution") return mutate(state, input.revenueProductId, (record) => recordExecution(record, input.input));
...
async function executeAction(env, revenueProductId, input) {
  try {
    ...
    const job = (Array.isArray(product.productionJobs) ? product.productionJobs : []).find((item) => item.jobId === jobId);
    if (!job) throw storeError("REVENUE_JOB_NOT_FOUND", "Revenue production job was not found.", 404);
    const execution = await executeKairosRevenueJob(product, job, env, input);
    const receipt = createKairosRevenueExecutionReceipt(execution, input.asset || {});
    const persisted = await forward(env, { operation: "record-execution", revenueProductId, input: { execution: summarizeExecution(execution), receipt } });
    const persistedPayload = await persisted.json();
    return json({ success: persisted.ok, execution, receipt, product: persistedPayload.product, builds: builds() }, persisted.status);
  } catch (error) { return failure(error); }
}

async function create(state, input) { try { const product = { ...buildKairosRevenueProduct(input), productionJobs: Object.freeze([]), executionReceipts: Object.freeze([]), projectId: clean(input.projectId, 180) || null, environment: clean(input.environment, 80), commitSha: clean(input.commitSha, 80) || null, createdAt: now(), updatedAt: now() }; const records = await load(state); if (records.some((item) => item.revenueProductId === product.revenueProductId)) return json({ success: false, error: { code: "REVENUE_PRODUCT_EXISTS", message: "Revenue product already exists." } }, 409); records.push(product); await save(state, records); return json({ success: true, product, builds: builds() }, 201); } catch (error) { return failure(error); } }
...
function recordExecution(record, input) { const receipt = input.receipt || {}; const receipts = [...(Array.isArray(record.executionReceipts) ? record.executionReceipts : []), receipt].slice(-100); return { ...record, executionReceipts: Object.freeze(receipts), lastExecution: Object.freeze(input.execution || {}), updatedAt: now() }; }
...
function rebuild(record) { const rebuilt = buildKairosRevenueProduct({ ...record, assets: record.assets, revenueProductId: record.revenueProductId, approval: record.approval }); return { ...record, ...rebuilt, productionJobs: record.productionJobs, executionReceipts: record.executionReceipts, createdAt: record.createdAt, updatedAt: now() }; }
async function exportAll(state) { const products = sorted(await load(state)); return json({ success: true, exportVersion: "kairos-revenue-product-export-v3", generatedAt: now(), count: products.length, products, commerceMutationIncluded: false, externalPublicationIncluded: false, deploymentExecutionIncluded: false, builds: builds() }); }
...
function builds() { return { engine: KAIROS_REVENUE_ENGINE_BUILD, jobs: KAIROS_REVENUE_PRODUCTION_JOBS_BUILD, assets: KAIROS_REVENUE_ASSET_REGISTRATION_BUILD, handoff: KAIROS_SHOPIFY_PUBLICATION_HANDOFF_BUILD, executor: KAIROS_OPENAI_REVENUE_EXECUTOR_BUILD, receipts: KAIROS_REVENUE_EXECUTION_RECEIPT_BUILD, store: KAIROS_REVENUE_PRODUCT_STORE_BUILD }; }
