...
import {
  ...
  PRODUCTS_TEMPLATE_FILE,
  PRODUCTS_TEMPLATE_SOURCE,
  PRODUCTS_SECTION_FILE,
  PRODUCTS_SECTION_SOURCE,
  PRODUCTS_CSS_FILE,
  PRODUCTS_CSS_SOURCE,
} from "./kairos-canonical-homepage-source-20260718.js";

export const KAIROS_PRODUCTS_LANDING_BUILD = "kairos-products-landing-publisher-20260718-1";
export const PRODUCTS_BUILD_PATH = "/api/shopify/staging/products-landing/build";
export const PRODUCTS_STAGING_CONFIRMATION = "BUILD_CANONICAL_MMG_PRODUCTS_STAGING";
export const PRODUCTS_PUBLISH_CONFIRMATION = "PUBLISH_CANONICAL_MMG_PRODUCTS_LIVE";

const MANAGED_FILES = [ECOSYSTEM_NAV_FILE, PRODUCTS_TEMPLATE_FILE, PRODUCTS_SECTION_FILE, PRODUCTS_CSS_FILE];
const SHOPIFY_TIMEOUT_MS = 25_000;
const READ_BACK_ATTEMPTS = 10;
const READ_BACK_DELAY_MS = 450;
const tokenCache = new Map();

export async function handleProductsLandingBuild(request, env) {
  const url = new URL(request.url);
  if (request.method !== "POST" || url.pathname !== PRODUCTS_BUILD_PATH) return null;
  const payload = await safeRequestJSON(request.clone());
  const mode = payload?.mode === "publish" ? "publish" : "build";
  const confirmation = mode === "publish" ? PRODUCTS_PUBLISH_CONFIRMATION : PRODUCTS_STAGING_CONFIRMATION;
  if (payload?.confirmation !== confirmation) throw httpError(403, "products_landing_confirmation_required", `Provide the exact confirmation phrase: ${confirmation}.`);

  const inspection = await inspectStagingSource(null, request, env, KAIROS_PRODUCTS_LANDING_BUILD, MANAGED_FILES);
  ...
  if (mode === "build") {
    ...
    return json({
      status: "completed",
      build: KAIROS_PRODUCTS_LANDING_BUILD,
      mode,
      summary: "Kairos installed the canonical Products landing page and ecosystem navigation into verified Kairos Staging and confirmed exact source read-back.",
      preview: { url: `${storefrontOrigin(env)}/pages/products?preview_theme_id=${themeID(evidence.stagingTheme.gid)}`, theme: evidence.stagingTheme },
      files: candidates.map(({ content, ...file }) => file),
      safeguards: { stagingOnly: true, mainThemeMutation: false, exactReadBack: true, productDiscoveryArchitectureVerified: true },
    });
  }
  ...
  return json({
    ...
    build: KAIROS_PRODUCTS_LANDING_BUILD,
    mode,
    summary: "Kairos promoted the verified canonical Products landing page and ecosystem navigation to Shopify MAIN and confirmed exact source read-back.",
    production: { url: `${storefrontOrigin(env)}/pages/products`, publishedTheme: evidence.mainTheme, publishedThemeChanged: true },
    files: candidates.map(({ content, ...file }) => file),
    verification: { stagingVerifiedBeforePublish: true, publishedThemeReadBackVerified: true, exactReadBack: true },
    safeguards: { stagingOnly: false, mainThemeMutation: true, sourceBundlePromotedWithoutReconstruction: true, productDiscoveryArchitectureVerified: true },
  });
}
...
async function prepareCandidates() {
  const candidates = [
    ...
    { filename: PRODUCTS_TEMPLATE_FILE, content: PRODUCTS_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: PRODUCTS_SECTION_FILE, content: PRODUCTS_SECTION_SOURCE, readBackVerification: "exact-bytes" },
    { filename: PRODUCTS_CSS_FILE, content: PRODUCTS_CSS_SOURCE, readBackVerification: "exact-bytes" },
  ];
...
async function verifyThemeReadBack(env, themeGid, candidates, label) {
  ...
  throw httpError(502, "products_landing_readback_mismatch", `Shopify did not expose the current ${label} Products landing revision. Observed ${lastObserved.join(", ") || "no readable files"}.${detail}`);
}
...
async function readThemeFiles(env, themeGid, filenames) {
  ...
  if (errors.length) throw httpError(502, "products_theme_file_read_failed", errors.map(error => error.code).join(", "));
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Products-Landing": KAIROS_PRODUCTS_LANDING_BUILD, "X-Content-Type-Options": "nosniff" } }); }
