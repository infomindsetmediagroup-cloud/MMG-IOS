...
export const KAIROS_THEME_MENU_HOTFIX_BUILD = "kairos-theme-menu-hotfix-20260718-1";
...
export async function handleThemeMenuHotfixPublish(request, env) {
  ...
  if (!before) throw httpError(409, "theme_layout_unavailable", `${LAYOUT_FILE} was not readable from Shopify MAIN.`);
  ...
  if (readBack !== after) throw httpError(502, "theme_menu_hotfix_readback_mismatch", "Shopify did not expose the exact menu hotfix revision.");

  return json({
    status: "completed",
    build: KAIROS_THEME_MENU_HOTFIX_BUILD,
    summary: "Published the corrected MMG navigation directly into Shopify MAIN without menu API scope.",
...
(()=>{"use strict";const BUILD="${KAIROS_THEME_MENU_HOTFIX_BUILD}";const NAV=${navJSON};const esc=v=>String(v).replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"})[c]);const children=g=>g[1].map(x=>'<a href="'+esc(x[1])+'">'+esc(x[0])+'</a>').join('');const drawer='<nav class="mmg-hotfix-drawer" data-mmg-menu="'+BUILD+'">'+NAV.map(g=>'<details><summary>'+esc(g[0])+'</summary><div>'+children(g)+'</div></details>').join('')+'</nav>';const desktop='<ul class="mmg-hotfix-desktop" data-mmg-menu="'+BUILD+'">'+NAV.map(g=>'<li><details><summary>'+esc(g[0])+'</summary><ul>'+g[1].map(x=>'<li><a href="'+esc(x[1])+'">'+esc(x[0])+'</a></li>').join('')+'</ul></details></li>').join('')+'</ul>';function install(){let changed=false;for(const target of document.querySelectorAll('.menu-drawer__navigation')){if(target.dataset.mmgHotfix!==BUILD){target.innerHTML=drawer;target.dataset.mmgHotfix=BUILD;changed=true}}for(const target of document.querySelectorAll('.header__inline-menu')){if(target.dataset.mmgHotfix!==BUILD){target.innerHTML=desktop;target.dataset.mmgHotfix=BUILD;changed=true}}document.documentElement.dataset.mmgThemeMenuHotfix=BUILD;return changed}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();let n=0;const timer=setInterval(()=>{n++;install();if(n>=80)clearInterval(timer)},125);new MutationObserver(install).observe(document.documentElement,{childList:true,subtree:true});})();
</script>\n${MARKER_END}`;
}
...
function stripBlock(source) {
  const a = source.indexOf(MARKER_START); const b = source.indexOf(MARKER_END);
  return a >= 0 && b > a ? source.slice(0, a) + source.slice(b + MARKER_END.length) : source;
}
