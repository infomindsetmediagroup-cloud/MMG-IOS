...
export const KAIROS_THEME_MENU_HOTFIX_BUILD = "kairos-theme-menu-hotfix-20260718-3";
...
export async function handleThemeMenuHotfixPublish(request, env) {
  ...
  if (payload?.confirmation !== THEME_MENU_HOTFIX_CONFIRMATION) throw httpError(403, "theme_menu_hotfix_confirmation_required", `Provide the exact confirmation phrase: ${THEME_MENU_HOTFIX_CONFIRMATION}.`);
  const config = readConfig(env);
  const auth = await resolveToken(config, env);
  const identity = await getShopIdentity(config, auth);
  const observedHosts = new Set([normalizeHost(identity?.primaryDomain?.host), normalizeHost(identity?.primaryDomain?.url), normalizeHost(identity?.myshopifyDomain)].filter(Boolean));
  if (!observedHosts.has(EXPECTED_STOREFRONT_HOST)) throw httpError(409, "shop_domain_identity_mismatch", `Connected Shopify shop does not own ${EXPECTED_STOREFRONT_HOST}. Observed: ${[...observedHosts].join(", ") || "none"}.`);
  ...
  return json({ status: "completed", build: KAIROS_THEME_MENU_HOTFIX_BUILD, summary: "Published selector-agnostic canonical MMG navigation into verified Shopify MAIN theme.", shop: { name: identity?.name || null, myshopifyDomain: identity?.myshopifyDomain || null, primaryDomain: identity?.primaryDomain || null, expectedStorefrontHost: EXPECTED_STOREFRONT_HOST, domainIdentityVerified: true }, theme: { id: theme.id, name: theme.name, role: theme.role }, verification: { exactThemeReadBack: true, menuApiScopeRequired: false, knowledgeLibraryRemoved: !after.includes('>Knowledge Library<'), companyScopeRestricted: true, inlineReleaseMarkerPresent: after.includes(KAIROS_THEME_MENU_HOTFIX_BUILD), shopDomainIdentityVerified: true, mainThemeRoleVerified: String(theme.role).toUpperCase() === "MAIN", targetThemeIdEmbedded: after.includes(theme.id), selectorAgnosticReplacement: true } });
}
...
.mmg-hotfix-desktop,.mmg-hotfix-drawer{color:inherit}.mmg-hotfix-desktop{display:flex;align-items:center;gap:2rem;list-style:none;margin:0;padding:0}.mmg-hotfix-desktop>li{position:relative}.mmg-hotfix-desktop a,.mmg-hotfix-desktop summary,.mmg-hotfix-drawer a,.mmg-hotfix-drawer summary{color:inherit;text-decoration:none;cursor:pointer;list-style:none}.mmg-hotfix-desktop summary::-webkit-details-marker,.mmg-hotfix-drawer summary::-webkit-details-marker{display:none}.mmg-hotfix-desktop ul{position:absolute;z-index:9999;top:100%;left:0;min-width:24rem;background:#fff;color:#111;border:1px solid rgba(0,0,0,.12);padding:1rem;list-style:none}.mmg-hotfix-desktop ul a{display:block;padding:.8rem 1rem}.mmg-hotfix-drawer{display:grid}.mmg-hotfix-drawer details{border-bottom:1px solid rgba(0,0,0,.12)}.mmg-hotfix-drawer summary{display:flex;justify-content:space-between;padding:1.25rem 1rem}.mmg-hotfix-drawer div{display:grid;padding:0 1rem 1rem}.mmg-hotfix-drawer a{padding:.65rem 0}@media(max-width:989px){.mmg-hotfix-desktop{display:none!important}}@media(min-width:990px){.mmg-hotfix-drawer{display:none!important}}
</style>
<script id="mmg-theme-menu-hotfix-script">
(()=>{"use strict";const BUILD="${KAIROS_THEME_MENU_HOTFIX_BUILD}",NAV=${navJSON},esc=v=>String(v).replace(/[&<>\"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"})[c]),children=g=>g[1].map(x=>'<a href="'+esc(x[1])+'">'+esc(x[0])+'</a>').join(''),drawer='<nav class="mmg-hotfix-drawer" data-mmg-menu="'+BUILD+'">'+NAV.map(g=>'<details><summary>'+esc(g[0])+'</summary><div>'+children(g)+'</div></details>').join('')+'</nav>',desktop='<ul class="mmg-hotfix-desktop" data-mmg-menu="'+BUILD+'">'+NAV.map(g=>'<li><details><summary>'+esc(g[0])+'</summary><ul>'+g[1].map(x=>'<li><a href="'+esc(x[1])+'">'+esc(x[0])+'</a></li>').join('')+'</ul></details></li>').join('')+'</ul>',legacy=/\b(Catalog|Knowledge Library)\b/i;function score(el){const t=(el.innerText||"").replace(/\s+/g," ");let s=0;if(/Catalog/i.test(t))s+=3;if(/Knowledge Library/i.test(t))s+=4;if(/Publishing Services/i.test(t))s++;if(/Customer Portal/i.test(t))s++;if(/Company/i.test(t))s++;return s}function replaceTarget(el){if(!el||el.dataset?.mmgHotfix===BUILD)return;const mobile=matchMedia('(max-width:989px)').matches||/drawer|mobile|menu/i.test(el.className||"");el.innerHTML=mobile?drawer:desktop;el.dataset.mmgHotfix=BUILD}function install(){const candidates=[...document.querySelectorAll('header nav,header ul,header [role="navigation"],nav,[class*="menu"],[id*="menu"]')].filter(el=>legacy.test(el.innerText||""));candidates.sort((a,b)=>score(b)-score(a));for(const el of candidates.slice(0,6))replaceTarget(el);for(const el of document.querySelectorAll('.menu-drawer__navigation'))replaceTarget(el);for(const el of document.querySelectorAll('.header__inline-menu'))replaceTarget(el);document.documentElement.dataset.mmgThemeMenuHotfix=BUILD}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();let n=0;const timer=setInterval(()=>{install();if(++n>=120)clearInterval(timer)},100);new MutationObserver(install).observe(document.documentElement,{childList:true,subtree:true});})();
</script>\n${MARKER_END}`;
}
function injectBlock(source, block) { const stripped = stripBlock(source); return /<\/body>/i.test(stripped) ? stripped.replace(/<\/body>/i, `${block}\n</body>`) : `${stripped}\n${block}`; }
