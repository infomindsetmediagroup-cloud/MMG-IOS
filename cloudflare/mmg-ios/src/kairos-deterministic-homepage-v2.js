export function buildDeterministicHomepagePackage(document, objective) {
  ...
  return {
    ...
    changes: [{
      ...
      purpose: "Update only verified text settings while preserving the rendered homepage.",
      changeType: "modify",
      instructions: [
        "Update only safe existing text settings.",
        "Preserve section IDs, block IDs, types, links, products, imagery, colors, typography, spacing, and integrations.",
        "Do not create or replace sections, blocks, stylesheets, or Liquid structures.",
      ],
      expectedOutcome: "A content-only homepage update with no visual redesign.",
    }],
    risks: ["Custom setting names are accepted only when they pass the strict non-visual string-field filter."],
    acceptanceCriteria: [
      "Section IDs, block IDs, section types, block types, order, imagery, links, and visual settings remain intact.",
      "Only approved string content fields change.",
      "The resulting templates/index.json is valid and semantically verified after Shopify read-back.",
      "The live theme remains unchanged.",
    ],
    rollbackPlan: [
      "Preserve the exact pre-change templates/index.json from Kairos Staging.",
      "Rollback requires separate approval and restores only that file on staging.",
    ],
    evidenceNotes: notes,
    objective,
  };
