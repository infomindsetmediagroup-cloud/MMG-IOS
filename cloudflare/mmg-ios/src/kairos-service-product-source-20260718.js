...
<section class="mmg-sp" data-product-id="{{ product.id }}">
  <nav class="mmg-sp__crumbs" aria-label="Breadcrumb"><a href="/">Home</a><span>/</span><a href="/pages/publishing-services">Services</a><span>/</span><span aria-current="page">{{ product.title }}</span></nav>
  <header class="mmg-sp__hero">
    <div class="mmg-sp__intro"><p class="mmg-sp__eyebrow">{{ section.settings.eyebrow }}</p><h1>{{ product.title }}</h1><p class="mmg-sp__promise">{% if product.metafields.custom.listing_subtitle != blank %}{{ product.metafields.custom.listing_subtitle }}{% else %}{{ section.settings.promise }}{% endif %}</p><div class="mmg-sp__description rte">{{ product.description }}</div><div class="mmg-sp__trust"><span>Defined deliverables</span><span>Secure checkout</span><span>Guided onboarding</span></div></div>
    <div class="mmg-sp__visual">{% if product.featured_media %}{{ product.featured_media | image_url: width: 1400 | image_tag: loading: 'eager', widths: '500,800,1100,1400', sizes: '(max-width: 900px) 92vw, 42vw', alt: product.featured_media.alt | default: product.title }}{% else %}<div class="mmg-sp__placeholder"><span>Mindset Media Group</span><strong>{{ product.title }}</strong></div>{% endif %}</div>
  </header>
  <section class="mmg-sp__tiers" id="service-levels">
    ...
    {% form 'product', product, class: 'mmg-sp__form', id: 'MMGServiceForm' %}
      ...
      <div class="mmg-sp__tier-grid" role="radiogroup" aria-label="Service level">
        {% for variant in product.variants %}
          ...
          <label class="mmg-sp__tier{% if variant.id == selected_variant.id %} is-selected{% endif %}{% unless variant.available %} is-unavailable{% endunless %}" data-service-tier>
            ...
            <h3>{{ variant.title }}</h3><div class="mmg-sp__price">{{ variant.price | money }}</div>
      ...
      <div class="mmg-sp__purchase"><div><span>Selected service</span><strong data-service-selection>{{ selected_variant.title }} · {{ selected_variant.price | money }}</strong></div><button type="submit" name="add" {% unless selected_variant.available %}disabled{% endunless %}>Add selected service to cart</button></div>
    {% endform %}
  </section>
  <section class="mmg-sp__comparison"><div class="mmg-sp__heading"><p class="mmg-sp__eyebrow">Scope clarity</p><h2>{{ section.settings.comparison_heading }}</h2></div><div class="mmg-sp__table-wrap"><table><thead><tr><th>Capability</th>{% for variant in product.variants %}<th>{{ variant.title }}</th>{% endfor %}</tr></thead><tbody><tr><th>Professional project onboarding</th>{% for variant in product.variants %}<td>Included</td>{% endfor %}</tr><tr><th>Editorial and structural review</th>{% for variant in product.variants %}<td>{% if variant.title contains 'Starter' %}Core{% else %}Expanded{% endif %}</td>{% endfor %}</tr><tr><th>Production-ready formatting</th>{% for variant in product.variants %}<td>Included</td>{% endfor %}</tr><tr><th>Customer Portal project tracking</th>{% for variant in product.variants %}<td>Included</td>{% endfor %}</tr></tbody></table></div><p class="mmg-sp__scope-note">Exact deliverables, eligibility, page limits, and add-ons remain governed by the product listing and project agreement.</p></section>
  <section class="mmg-sp__process"><div class="mmg-sp__heading"><p class="mmg-sp__eyebrow">From purchase to delivery</p><h2>{{ section.settings.process_heading }}</h2></div><ol>{% if process_items != blank %}{% for item in process_items %}<li><span>{{ forloop.index }}</span><div><h3>{{ item.title }}</h3><p>{{ item.description }}</p></div></li>{% endfor %}{% else %}<li><span>1</span><div><h3>Purchase and onboarding</h3><p>Select the appropriate tier, complete checkout, and receive project intake instructions.</p></div></li><li><span>2</span><div><h3>Submit project materials</h3><p>Upload manuscripts, references, images, and required assets through the approved workflow.</p></div></li><li><span>3</span><div><h3>Production and review</h3><p>Your project advances through the defined editorial, design, formatting, and quality-control stages.</p></div></li><li><span>4</span><div><h3>Delivery and support</h3><p>Approved deliverables are organized in your customer workspace with applicable revision terms.</p></div></li>{% endif %}</ol></section>
  <section class="mmg-sp__policy"><div><p class="mmg-sp__eyebrow">Customer protection</p><h2>{{ section.settings.policy_heading }}</h2><p>Eligible manuscript services include one complimentary editorial rewrite request within 30 days of delivery. Scope changes, new source material, added pages, and work outside the purchased tier may require a revised quote.</p></div><a href="/pages/customer-service-policies">Review service policies</a></section>
  <section class="mmg-sp__faq"><div class="mmg-sp__heading"><p class="mmg-sp__eyebrow">Before you begin</p><h2>{{ section.settings.faq_heading }}</h2></div>{% if faq_items != blank %}{% for item in faq_items %}<details><summary>{{ item.question }}</summary><div>{{ item.answer }}</div></details>{% endfor %}{% else %}<details><summary>What happens after purchase?</summary><div>You receive onboarding instructions describing the project intake process and required materials.</div></details><details><summary>How do I choose the right tier?</summary><div>Choose based on page count, project complexity, required deliverables, and the amount of editorial or production support needed.</div></details><details><summary>Are research, cover design, or other add-ons included?</summary><div>Only when explicitly listed in the selected tier or purchased as an approved add-on.</div></details>{% endif %}</section>
</section>
<script>
(() => {
