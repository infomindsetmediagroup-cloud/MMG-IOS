...
export const KAIROS_SERVICES_LANDING_BUILD = "kairos-services-landing-publisher-20260718-1";
export const SERVICES_BUILD_PATH = "/api/shopify/staging/services-landing/build";
export const SERVICES_STAGING_CONFIRMATION = "BUILD_CANONICAL_MMG_SERVICES_STAGING";
export const SERVICES_PUBLISH_CONFIRMATION = "PUBLISH_CANONICAL_MMG_SERVICES_LIVE";

const MANAGED_FILES = [ECOSYSTEM_NAV_FILE, SERVICES_TEMPLATE_FILE, SERVICES_SECTION_FILE, SERVICES_CSS_FILE];
const SHOPIFY_TIMEOUT_MS = 25_000;
const READ_BACK_ATTEMPTS = 10;
const READ_BACK_DELAY_MS = 450;
const tokenCache = new Map();

export async function handleServicesLandingBuild(request, env) {
  const url = new URL(request.url);
  if (request.method !== "POST" || url.pathname !== SERVICES_BUILD_PATH) return null;
  const payload = await safeRequestJSON(request.clone());
  const mode = payload?.mode === "publish" ? "publish" : "build";
  const confirmation = mode === "publish" ? SERVICES_PUBLISH_CONFIRMATION : SERVICES_STAGING_CONFIRMATION;
  if (payload?.confirmation !== confirmation) {
    throw httpError(403, "services_landing_confirmation_required", `Provide the exact confirmation phrase: ${confirmation}.`);
  }

  const inspection = await inspectStagingSource(null, request, env, KAIROS_SERVICES_LANDING_BUILD, MANAGED_FILES);
  ...
  if (mode === "build") {
    ...
    return json({
      status: "completed",
      build: KAIROS_SERVICES_LANDING_BUILD,
      mode,
      summary: "Kairos installed the canonical Services landing page and ecosystem navigation into verified Kairos Staging and confirmed exact source read-back.",
      preview: { url: `${storefrontOrigin(env)}/pages/publishing-services?preview_theme_id=${themeID(evidence.stagingTheme.gid)}`, theme: evidence.stagingTheme },
  ...
  return json({
    ...
    build: KAIROS_SERVICES_LANDING_BUILD,
    mode,
    summary: "Kairos promoted the verified canonical Services landing page and ecosystem navigation to Shopify MAIN and confirmed exact source read-back.",
    production: { url: `${storefrontOrigin(env)}/pages/publishing-services`, publishedTheme: evidence.mainTheme, publishedThemeChanged: true },
    files: candidates.map(({ content, ...file }) => file),
...
async function prepareCandidates() {
  const candidates = [
    ...
    { filename: SERVICES_TEMPLATE_FILE, content: SERVICES_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: SERVICES_SECTION_FILE, content: SERVICES_SECTION_SOURCE, readBackVerification: "exact-bytes" },
    { filename: SERVICES_CSS_FILE, content: SERVICES_CSS_SOURCE, readBackVerification: "exact-bytes" },
  ];
...
async function verifyThemeReadBack(env, themeGid, candidates, label) {
  ...
  throw httpError(502, "services_landing_readback_mismatch", `Shopify did not expose the current ${label} Services landing revision. Observed ${lastObserved.join(", ") || "no readable files"}.${detail}`);
}
...
async function readThemeFiles(env, themeGid, filenames) {
  ...
  if (errors.length) throw httpError(502, "services_theme_file_read_failed", errors.map(error => error.code).join(", "));
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Services-Landing": KAIROS_SERVICES_LANDING_BUILD, "X-Content-Type-Options": "nosniff" } }); }
