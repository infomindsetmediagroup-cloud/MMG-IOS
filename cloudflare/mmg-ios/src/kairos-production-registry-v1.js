const BUILD = "kairos-production-registry-20260713-1";
...
export async function handleProductionRegistry(request, env) {
  ...
  if (!env.KAIROS_PROJECTS) return json({ status: "needs-configuration", error: { code: "production_registry_unavailable", message: "The persistent Kairos project runtime is not configured." } }, 503);
  ...
  return stub.fetch(new Request(`https://kairos.internal/registry${suffix}${url.search}`, request));
}
...
export async function handleRegistryObjectRequest(state, request) {
  ...
  if (match && request.method === "GET") {
    ...
    return project ? json({ status: "ready", build: BUILD, project }) : json({ status: "not-found", error: { code: "production_project_not_found", message: "The production project was not found." } }, 404);
  }
  ...
  if (match && request.method === "PATCH") {
    ...
    records[current.projectId] = { ...current, ...next, updatedAt: new Date().toISOString(), revision: Number(current.revision || 0) + 1 };
    await state.storage.put("production-registry", records);
  ...
  if (match && request.method === "DELETE") {
    ...
    const current = records[match[1]];
    if (!current) return json({ status: "not-found", error: { code: "production_project_not_found", message: "The production project was not found." } }, 404);
    records[current.projectId] = { ...current, status: "archived", archivedAt: new Date().toISOString(), updatedAt: new Date().toISOString(), revision: Number(current.revision || 0) + 1 };
    await state.storage.put("production-registry", records);
...
function validDate(value) { const text = String(value || ""); return /^\d{4}-\d{2}-\d{2}T/.test(text) ? text : null; }
function clamp(value, min, max) { return Math.min(max, Math.max(min, Number.isFinite(value) ? value : min)); }
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-Kairos-Registry": BUILD } }); }
