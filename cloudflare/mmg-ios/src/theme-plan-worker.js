...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname !== "/api/theme-plan" || request.method !== "POST") return reconciledWorker.fetch(request, env, ctx);
...
async function fallback(request, env) {
  ...
  const themeData = await gql(shop, `query { themes(first:1, roles:[MAIN]) { nodes { id name role processing processingFailed } } }`);
  const themeNode = themeData?.themes?.nodes?.[0];
  if (!themeNode || themeNode.role !== "MAIN") throw problem(502, "main_theme_unavailable", "Shopify did not return the published main theme.");
  if (themeNode.processing || themeNode.processingFailed) throw problem(409, "main_theme_processing", "The published Shopify theme is still processing or failed processing.");

  const idMatch = String(themeNode.id || "").match(/^gid:\/\/shopify\/OnlineStoreTheme\/(\d+)$/);
  if (!idMatch) throw problem(502, "invalid_theme_id", "Shopify returned an invalid theme ID.");
  const themeId = idMatch[1];

  const fileData = await gql(shop, `query($id:ID!,$names:[String!]) { theme(id:$id) { files(first:5, filenames:$names) { nodes { filename body { ... on OnlineStoreThemeFileBodyText { content } ... on OnlineStoreThemeFileBodyBase64 { contentBase64 } ... on OnlineStoreThemeFileBodyUrl { url } } } userErrors { code filename } } } }`, {
    id: themeNode.id,
    names: ["layout/theme.liquid", ...CSS_KEYS],
  });

  const fileConnection = fileData?.theme?.files;
  const readErrors = (fileConnection?.userErrors || []).filter(error => error?.code && error.code !== "NOT_FOUND");
  if (readErrors.length) throw problem(502, "theme_file_read_failed", "Shopify could not read the required theme files.");

  const files = [];
  for (const node of fileConnection?.nodes || []) {
  ...
  if (!/template-index|template-\s*{{\s*(?:request\.page_type|template\.name)/i.test(layout.value)) {
    throw problem(409, "homepage_scope_unverified", "The published layout does not prove homepage-only scope.");
  }
  ...
  return json({
    ...
    recommendedChanges: ["Add consistent homepage section spacing.", "Improve heading and paragraph wrapping.", "Preserve mobile safe areas and accessible control height."],
    affectedAssets: [stylesheet.key],
    expectedBenefits: ["Clearer homepage progression.", "Improved readability and mobile tap comfort.", "No intended impact outside the homepage."],
    risks: ["Existing section-specific spacing may require visual review after execution."],
    ...
    sourceEvidence: { adapter: "graphql-admin-verified-homepage-fallback-v2", themeId, themeName: themeNode.name || "Published theme", role: "main", homepageSelector: ".template-index", objective, files: [{ key: layout.key, sha256: sha(layout.value) }, { key: stylesheet.key, sha256: sha(stylesheet.value) }] },
  });
}
...
async function shopify(env) {
  ...
  if (!token && clientId && clientSecret) {
    ...
    if (cached?.expiresAt > Date.now()) token = cached.token;
    else {
      const response = await fetch(`https://${domain}/admin/oauth/access_token`, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ grant_type: "client_credentials", client_id: clientId, client_secret: clientSecret }) });
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Runtime": "theme-plan-worker-v2" } }); }
