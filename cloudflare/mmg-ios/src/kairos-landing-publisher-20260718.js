...
import {
  ...
  KAIROS_TEMPLATE_FILE,
  KAIROS_TEMPLATE_SOURCE,
  KAIROS_SECTION_FILE,
  KAIROS_SECTION_SOURCE,
  KAIROS_CSS_FILE,
  KAIROS_CSS_SOURCE,
} from "./kairos-canonical-homepage-source-20260718.js";

export const KAIROS_LANDING_BUILD = "kairos-landing-publisher-20260718-1";
export const KAIROS_BUILD_PATH = "/api/shopify/staging/kairos-landing/build";
export const KAIROS_STAGING_CONFIRMATION = "BUILD_CANONICAL_MMG_KAIROS_STAGING";
export const KAIROS_PUBLISH_CONFIRMATION = "PUBLISH_CANONICAL_MMG_KAIROS_LIVE";

const MANAGED_FILES = [ECOSYSTEM_NAV_FILE, KAIROS_TEMPLATE_FILE, KAIROS_SECTION_FILE, KAIROS_CSS_FILE];
const SHOPIFY_TIMEOUT_MS = 25_000;
const READ_BACK_ATTEMPTS = 10;
const READ_BACK_DELAY_MS = 450;
const tokenCache = new Map();

export async function handleKairosLandingBuild(request, env) {
  const url = new URL(request.url);
  if (request.method !== "POST" || url.pathname !== KAIROS_BUILD_PATH) return null;
  const payload = await safeRequestJSON(request.clone());
  const mode = payload?.mode === "publish" ? "publish" : "build";
  const confirmation = mode === "publish" ? KAIROS_PUBLISH_CONFIRMATION : KAIROS_STAGING_CONFIRMATION;
  if (payload?.confirmation !== confirmation) throw httpError(403, "kairos_landing_confirmation_required", `Provide the exact confirmation phrase: ${confirmation}.`);

  const inspection = await inspectStagingSource(null, request, env, KAIROS_LANDING_BUILD, MANAGED_FILES);
  ...
  if (mode === "build") {
    ...
    return json({
      status: "completed",
      build: KAIROS_LANDING_BUILD,
      mode,
      summary: "Kairos installed the canonical Kairos landing page and ecosystem navigation into verified Kairos Staging and confirmed exact source read-back.",
      preview: { url: `${storefrontOrigin(env)}/pages/kairos?preview_theme_id=${themeID(evidence.stagingTheme.gid)}`, theme: evidence.stagingTheme },
      files: candidates.map(({ content, ...file }) => file),
      safeguards: { stagingOnly: true, mainThemeMutation: false, exactReadBack: true, publicCapabilityClaimsBounded: true },
    });
  }
  ...
  return json({
    ...
    build: KAIROS_LANDING_BUILD,
    mode,
    summary: "Kairos promoted the verified canonical Kairos landing page and ecosystem navigation to Shopify MAIN and confirmed exact source read-back.",
    production: { url: `${storefrontOrigin(env)}/pages/kairos`, publishedTheme: evidence.mainTheme, publishedThemeChanged: true },
    files: candidates.map(({ content, ...file }) => file),
    verification: { stagingVerifiedBeforePublish: true, publishedThemeReadBackVerified: true, exactReadBack: true },
    safeguards: { stagingOnly: false, mainThemeMutation: true, sourceBundlePromotedWithoutReconstruction: true, publicCapabilityClaimsBounded: true },
  });
}
...
async function prepareCandidates() {
  const candidates = [
    ...
    { filename: KAIROS_TEMPLATE_FILE, content: KAIROS_TEMPLATE_SOURCE, readBackVerification: "semantic-json" },
    { filename: KAIROS_SECTION_FILE, content: KAIROS_SECTION_SOURCE, readBackVerification: "exact-bytes" },
    { filename: KAIROS_CSS_FILE, content: KAIROS_CSS_SOURCE, readBackVerification: "exact-bytes" },
  ];
...
async function verifyThemeReadBack(env, themeGid, candidates, label) {
  ...
  throw httpError(502, "kairos_landing_readback_mismatch", `Shopify did not expose the current ${label} Kairos landing revision. Observed ${lastObserved.join(", ") || "no readable files"}.${detail}`);
}
...
async function readThemeFiles(env, themeGid, filenames) {
  ...
  if (errors.length) throw httpError(502, "kairos_theme_file_read_failed", errors.map(error => error.code).join(", "));
...
function json(value, status = 200) { return new Response(JSON.stringify(value), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-MMG-Kairos-Landing": KAIROS_LANDING_BUILD, "X-Content-Type-Options": "nosniff" } }); }
