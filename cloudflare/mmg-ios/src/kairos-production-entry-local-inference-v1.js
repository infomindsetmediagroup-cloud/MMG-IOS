...
const BUILD = "kairos-production-entry-tool-objectives-20260725-14";
...
export default {
  async fetch(request, env, ctx) {
    ...
    const kairosAPI = await handleGovernedKairosAPI(request.clone(), env, (governedRequest) => handleToolAwareKairosObjective(governedRequest, env, (toolAwareRequest) => handleContextualKairosAPI(toolAwareRequest, env, (contextualRequest) => handleKairosAPI(contextualRequest, env)))); if (kairosAPI) return stamp(kairosAPI);
