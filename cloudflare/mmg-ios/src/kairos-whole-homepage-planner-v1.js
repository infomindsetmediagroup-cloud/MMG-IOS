...
function assignJourneyZones(sections) {
  ...
  for (const section of sections) {
    ...
    const fallback = fallbackZone(section, sections.length);
    assignments.push({ ...section, zone: fallback });
  }
...
function addRoleOperations(target, zone, items, values, role) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    target.push({
      ...
      after,
      zone,
      reason: `Whole-page ${zone} ${role} ${index + 1}`,
    });
  }
}
...
async function buildTextPackage(source, operations) {
  ...
  if (templateStructureSignature(candidate) !== structureSignature) {
    throw httpError(409, "whole_homepage_structure_changed", "The whole-page text plan changed the Shopify homepage structure.");
  }
  ...
  return {
    ...
    files: [{
      filename: TEMPLATE_FILE,
      beforeSha256: source.templateFile.sha256,
      afterSha256,
      beforeSource: source.templateFile.content,
      candidateSource,
      structureSignature,
      operations: compact,
    }],
...
function normalize(value) {
  return String(value || "").toLowerCase().replace(/[™®]/g, "").replace(/[_–—-]+/g, " ").replace(/[^a-z0-9\s]/g, " ").replace(/\s+/g, " ").trim();
}
...
function json(value, status = 200) {
  return new Response(JSON.stringify(value), {
    ...
    headers: {
      ...
      "X-MMG-Runtime": KAIROS_WHOLE_HOMEPAGE_PLANNER_BUILD,
      "X-Kairos-Homepage-Planner-Mode": "deterministic-whole-homepage-source-plan",
      "X-Kairos-Template-Only-Mutation": "true",
      "X-Kairos-Workers-AI-Used": "false",
      "X-Kairos-Private-Runtime-Used": "false",
