...
assert.ok(index.includes("recovery-20260714-28"),"Executive simple mode build marker is not current.");
for(const marker of ["System Care","Approve & Start","Continue Fix","Resume","Finish","Kairos is ready","data-executive-command","approve-start","Open My Work","What Kairos is handling","In Progress","Waiting","Finished Today","data-close-my-work"])assert.ok(source.includes(marker),`Executive simple mode is missing: ${marker}`);
...
assert.ok(css.includes(".my-work-group"),"My Work groups are not styled.");
assert.ok(!source.includes("setInterval("),"Executive simple mode must not add polling.");
console.log(JSON.stringify({status:"ready",feature:"executive-simple-mode",technicalPanelsHidden:true,oneClickApproval:true,oneClickRecovery:true,oneClickFinish:true,executivePulse:true,simplifiedMyWork:true},null,2));