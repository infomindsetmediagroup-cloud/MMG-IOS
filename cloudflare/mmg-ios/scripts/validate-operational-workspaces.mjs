...
for(const marker of["kairos-viewport-stability-lock-20260715-1","window.scrollTo=function","window.scrollBy=function","Element.prototype.scrollIntoView=function","preventScroll:true","overflowAnchor","programmaticNavigationAllowed"])assert.ok(viewport.includes(marker),`Viewport lock missing ${marker}`);
console.log(JSON.stringify({status:"ready",validator:"kairos-operational-workspaces-20260715-11",workspacesVerified:verified.length,knowledgeParentComplete:true,contentParentComplete:true,singleParentCardRenderer:true,competingParentControllers:0,unsolicitedViewportMovementBlocked:true,explicitNavigationWindowBounded:true,telemetryDOMStable:true,structuralContracts:true,floatingControls:0},null,2));