...
describe("Kairos customer runtime projection store", () => {
  ...
  it("routes customer-scoped collection reads", async () => {
    let forwarded: any;
    const env = { KAIROS_PROJECTS: { idFromName: () => "id", get: () => ({ fetch: async (request: Request) => { forwarded = await request.json(); return new Response("{}"); } }) } };
    const request = new Request("https://example.com/api/kairos/customer/projects", { headers: { "cf-access-authenticated-user-email": "customer@example.com", "x-kairos-customer-id": "customer_1" } });
