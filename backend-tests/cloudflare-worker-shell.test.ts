...
describe("reconciled Cloudflare worker", () => {
  it("serves the bundled Command Center shell from the asset binding", async () => {
    const assets = { fetch: vi.fn(async () => new Response("<!doctype html><title>Kairos</title>", { status: 200, headers: { "content-type": "text/html" } })) };
    ...
    expect(response.headers.get("x-mmg-build")).toBe("command-center-reconciled-20260711-29");
