...
const probe = (complete: boolean, productStatus: "DRAFT" | "ACTIVE" = "DRAFT"): MMGCommerceDeploymentProbe => ({
...
const repository = (approval: boolean): MMGCommerceDeploymentRepository => {
  ...
  return {
    ...
    loadApproval: vi.fn().mockResolvedValue(
      approval
        ? {
            ...
            approvedActions: ["execute", "publish", "rollback"],
    ...
    beginRelease: vi.fn().mockImplementation(async () => ({ version, created: true })),
...
describe("MMG live commerce deployment service", () => {
  it("executes staging phases without pretending to publish", async () => {
    ...
    const gateway = {
      ...
      applyPhase: vi.fn().mockResolvedValue({ status: "completed", result: {} }),
      rollbackPhase: vi.fn(),
    };
    ...
    expect(
      gateway.applyPhase.mock.calls.some(([input]) => input.phase === "publication"),
    ).toBe(false);
  });
  ...
  it("publishes only after complete evidence and a distinct approval", async () => {
    const repo = repository(true);
    let published = false;
    const gateway = {
      probe: vi.fn().mockImplementation(async () =>
        probe(true, published ? "ACTIVE" : "DRAFT"),
      ),
    ...
    const response = await executeMMGCommerceDeploymentCommand({
      command: {
        ...
        releaseId: "release-20260720-publish",
        environment: "production",
        action: "publish",
        releaseCommitSha: releaseSha,
