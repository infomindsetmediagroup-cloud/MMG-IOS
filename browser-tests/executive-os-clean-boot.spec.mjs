...
const headerBanner = readFileSync(new URL("../web/kairos-dashboard/assets/kairos-header-brand.png", import.meta.url));
...
async function installDashboardRoutes(page, { delayCommandHubMs = 0 } = {}) {
  ...
  await page.route("https://kairos.test/**", async route => {
    ...
    if (path === "/assets/kairos-header-brand.png") {
...
test("default iPhone route restores the five-parent-card Kairos command dashboard", async ({ page }) => {
  ...
  await expect(page.locator(".app-header-image")).toHaveAttribute("src", /assets\/kairos-header-brand\.png/);
  ...
  }))).toEqual({ complete: true, naturalWidth: 1264, naturalHeight: 468 });
