...
test("the governed loader cannot request the retired manuscript asset cache key", async () => {
  const source = await readFile(loaderPath, "utf8");

  assert.match(source, /five-center-dashboard-local-production-20260731-5/);
...
test("the dashboard imports the canonical controller under a fully fresh Safari cache lineage", async () => {
  ...
  assert.match(bootstrap, /five-center-dashboard-local-production-20260731-5/);
  assert.match(bootstrap, /manuscript-local-production-controller-20260731-4/);
