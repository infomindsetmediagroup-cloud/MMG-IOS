...
export async function inspectStorefront(limit = DEFAULT_LIMIT): Promise<StorefrontInspection> {
  ...
  for (let index = 0; index < urls.length; index += concurrency) {
    ...
    const results = await Promise.all(batch.map(async (url) => {
      try { return await inspectPage(url); }
      catch (error) {
...
async function discoverSitemaps(url: string, sitemapUrls: string[], discovered: Set<string>, errors: Array<{ url: string; message: string }>, depth: number): Promise<void> {
  ...
  try {
    ...
    for (const location of locations) {
      ...
      if (/\.xml(?:\?|$)/i.test(normalized)) await discoverSitemaps(normalized, sitemapUrls, discovered, errors, depth + 1);
      else discovered.add(normalized);
    }
  } catch (error) {
...
async function inspectPage(url: string): Promise<PageInspection> {
  ...
  if (!contentType.includes("text/html")) return { url, status: response.status, finalUrl, h1: [], links: [], assets: [], issues: [...issues, "Non-HTML response"] };
  ...
  const assets = [...extractUrls(html, /<(?:img|script)\b[^>]*\bsrc=["']([^"']+)["']/gi, finalUrl), ...extractUrls(html, /<link\b[^>]*\bhref=["']([^"']+)["']/gi, finalUrl)];
  ...
  return { url, status: response.status, finalUrl, ...(title ? { title } : {}), ...(description ? { description } : {}), ...(canonical ? { canonical } : {}), h1, links: links.slice(0, 250), assets: [...new Set(assets)].slice(0, 250), issues };
}
...
async function safeFetch(url: string): Promise<Response> {
  ...
  return fetch(normalized, { redirect: "follow", headers: { Accept: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.1", "User-Agent": "MMG-Kairos-Storefront-Inspector/1.0" }, signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS) });
}
...
function metaContent(html: string, name: string): string | undefined {
  const patterns = [new RegExp(`<meta\\b[^>]*name=["']${name}["'][^>]*content=["']([^"']*)["'][^>]*>`, "i"), new RegExp(`<meta\\b[^>]*content=["']([^"']*)["'][^>]*name=["']${name}["'][^>]*>`, "i")];
  for (const pattern of patterns) { const value = firstText(html, pattern); if (value) return value; }
  return undefined;
}

function linkHref(html: string, rel: string): string | undefined {
  const patterns = [new RegExp(`<link\\b[^>]*rel=["'][^"']*${rel}[^"']*["'][^>]*href=["']([^"']+)["'][^>]*>`, "i"), new RegExp(`<link\\b[^>]*href=["']([^"']+)["'][^>]*rel=["'][^"']*${rel}[^"']*["'][^>]*>`, "i")];
  for (const pattern of patterns) { const match = html.match(pattern); if (match?.[1]) return decodeXml(match[1].trim()); }
  return undefined;
}
...
function extractUrls(html: string, pattern: RegExp, base: string): string[] {
  ...
  for (const match of html.matchAll(pattern)) {
    try { const url = new URL(decodeXml(match[1]), base); if (url.protocol === "http:" || url.protocol === "https:") urls.push(url.toString()); } catch {}
  }
  return [...new Set(urls)];
}

function cleanText(value: string): string { return decodeXml(value.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim()); }
