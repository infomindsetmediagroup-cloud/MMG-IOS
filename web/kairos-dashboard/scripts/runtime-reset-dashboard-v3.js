const BUILD = "kairos-runtime-reset-20260711-3";
const root = document.querySelector("#reset-dashboard");
const centers = [
  { id:"executive", eyebrow:"Executive Operations", title:"Direction, approvals, and operating priorities", description:"The executive layer will coordinate priorities, approvals, evidence, and controlled execution across the MMG ecosystem.", status:"Foundation Available", tone:"available", abilities:[
  ...
  { id:"shopify", eyebrow:"Shopify & Website", title:"Storefront, theme, products, and publishing", description:"Administrative work stays inside Shopify. Kairos performs bounded operations and brings evidence back into this Command Center.", status:"1 Capability Active", tone:"limited",
    workspace:[
      {title:"Open Shopify Admin",detail:"Primary administrative workspace for products, orders, content, apps, settings, and store operations.",href:"https://admin.shopify.com/"},
      {title:"Open Theme Editor",detail:"Administrative theme-customization workspace. This is not the customer-facing storefront.",href:"https://themindsetmediagroup.com/admin/themes/current/editor"}
    ],
    abilities:[
      {title:"Inspect live storefront",state:"Operational",detail:"Kairos performs a read-only inspection of the public homepage and sitemap and returns evidence here.",action:"inspect-storefront",label:"Inspect Live Storefront"},
    ...
    verification:[{title:"Open Live Storefront",detail:"Customer-facing view used only to verify what visitors actually see after approved changes.",href:"https://themindsetmediagroup.com"}]
  },
...
root.innerHTML = `
<section class="reset-hero"><p class="eyebrow">Kairos Command Center</p><h1>Clean rebuild. One verified capability at a time.</h1><p class="lead">The canonical Kairos shell is frozen. Parent operating centers are restored below, but only tested capabilities are interactive. Every new ability must complete the full evidence, approval, execution, verification, and recovery pipeline before it is marked operational.</p><div class="reset-badge">${BUILD}</div></section>
...
<section id="execution-panel" class="reset-panel execution-panel" hidden><div class="section-heading"><div><p class="eyebrow">Live Capability Evidence</p><h2 id="execution-title">Execution result</h2></div><span id="execution-status" class="status-pill limited">Working</span></div><div id="execution-result" class="execution-result" aria-live="polite"></div></section>
<section class="reset-panel doctrine-panel"><p class="eyebrow">Execution Standard</p><h2>No capability is operational without proof</h2><p>Each vertical must pass source validation, bounded execution, read-back verification, rollback preparation, explicit approval controls where required, and recorded evidence. The shell remains stable while capabilities are plugged into it individually.</p></section>`;

root.addEventListener("click", async event => {
  const button = event.target.closest("[data-action]");
  if (button && !button.disabled && button.dataset.action === "inspect-storefront") await inspectStorefront(button);
});

async function inspectStorefront(button) {
  const panel=document.querySelector("#execution-panel"), title=document.querySelector("#execution-title"), status=document.querySelector("#execution-status"), result=document.querySelector("#execution-result");
  panel.hidden=false; title.textContent="Inspect live storefront"; status.textContent="Working"; status.className="status-pill limited";
  result.innerHTML='<p class="lead compact">Reading the public homepage and sitemap. No Shopify mutation authority is used.</p>';
  button.disabled=true; button.textContent="Inspecting…"; panel.scrollIntoView({behavior:"smooth",block:"start"});
  try {
    ...
    status.textContent="Completed"; status.className="status-pill available";
    result.innerHTML=`<div class="evidence-summary"><strong>${escapeHTML(body.summary||"Storefront inspection completed.")}</strong><span>Read-only · ${escapeHTML(body.completedAt||"")}</span></div><div class="evidence-grid">${renderEvidence("Homepage",body?.evidence?.homepage||{})}${renderEvidence("Sitemap",body?.evidence?.sitemap||{})}</div><details class="evidence-details"><summary>View execution evidence</summary><pre>${escapeHTML(JSON.stringify(body,null,2))}</pre></details>`;
  } catch(error) { status.textContent="Needs Attention"; status.className="status-pill blocked"; result.innerHTML=`<p class="execution-error">${escapeHTML(error instanceof Error?error.message:"Storefront inspection failed.")}</p>`; }
  finally { button.disabled=false; button.textContent="Inspect Live Storefront"; }
}

function renderCenter(center){return `<article class="parent-card ${center.tone}"><div class="parent-card-header"><div><p class="eyebrow">${escapeHTML(center.eyebrow)}</p><h3>${escapeHTML(center.title)}</h3></div><span class="status-pill ${center.tone}">${escapeHTML(center.status)}</span></div><p class="parent-description">${escapeHTML(center.description)}</p>${renderLinkGroup("Workspace",center.workspace)}<div class="ability-list">${center.abilities.map(renderAbility).join("")}</div>${renderLinkGroup("Verification",center.verification,"verification-group")}</article>`}
