const BUILD = "kairos-publishing-experience-ui-20260723-2";
...
function scheduleEnhance() {
  ...
  queueMicrotask(()=>{ state.mountScheduled=false; void enhance(); });
}
...
async function load(projectId) {
  state.busy=true; state.phase="Loading publishing job…"; state.error=""; render();
  try {
    const response = await fetch(endpoint(projectId), { credentials:"include", cache:"no-store" });
    if (response.status === 404) { state.record=null; return; }
  ...
  } catch (error) { state.error=error?.message||"The publishing job could not be loaded."; }
  finally { state.busy=false; state.phase=""; render(); }
}

async function runPipeline(projectId) {
  return perform("Manufacturing the complete customer package…", async()=>{ state.record=await post(`${endpoint(projectId)}/run`,{}); });
}
async function approvePackage(projectId) {
  return perform("Approving and freezing the Asset Vault package…", async()=>{ state.record=await post(`/api/production-registry/manuscripts/${encodeURIComponent(projectId)}/experience/approve-package`,{ confirmation:PACKAGE_CONFIRMATION, actor:"MMG Executive" }); });
}
async function previewShopifyProduct(projectId) {
  return perform("Creating the actual Shopify draft, installing media, and verifying customer delivery…", async()=>{ state.record=await post(`${endpoint(projectId)}/shopify-draft`,{ confirmation:DRAFT_CONFIRMATION }); });
}
async function publishLive(projectId) {
  return perform("Publishing and verifying the approved Shopify product…", async()=>{ state.record=await post(`${endpoint(projectId)}/shopify-publish`,{ confirmation:LIVE_CONFIRMATION }); });
}
async function perform(phase, operation) {
  if (state.busy) return;
  state.busy=true; state.phase=phase; state.error=""; render();
  try { await operation(); }
  catch (error) { state.error=error?.message||"Kairos could not complete this operation."; }
  finally { state.busy=false; state.phase=""; render(); }
}
...
function render(section=document.querySelector("#manuscript-auto-pipeline")) {
  ...
  if (state.busy) return void (section.innerHTML=busyMarkup());
  if (!state.record) return void (section.innerHTML=intakeMarkup());
...
function liveMarkup() {
  const metadata=state.record.metadata||{}, shopify=state.record.shopify||{};
  const liveURL=metadata.liveURL||shopify.livePublication?.publication?.liveProbe?.finalURL||"";
  return `${stepper(5)}<p class="eyebrow">Published and verified</p><h3>${esc(metadata.title||"Product published")}</h3><div class="issue-list"><article><b>Production</b><p>Complete</p></article><article><b>Package</b><p>Approved</p></article><article><b>Shopify product</b><p>Published</p></article><article><b>Customer delivery</b><p>Connected</p></article></div>${liveURL?`<a class="manuscript-package" href="${esc(liveURL)}" target="_blank" rel="noopener">View Live Product</a>`:""}${errorMarkup()}`;
}
...
function handleClick(event) {
  const button=event.target instanceof Element?event.target.closest("[data-start-production], [data-approve-package], [data-preview-shopify], [data-publish-product]"):null;
