...
function reviewPanel(){const p=state.plan||{},plan=p.plan||{},changes=(plan.changes||[]).filter(x=>x.changeType!=="no-change");return `<div class="summary-card"><h4>${escapeHTML(p.summary||"Execution package prepared")}</h4><p>${escapeHTML(plan.strategy||"Kairos prepared a bounded staging change.")}</p><ul>${changes.slice(0,4).map(x=>`<li><strong>${escapeHTML(x.filename||"Website")}</strong> — ${escapeHTML(x.purpose||x.expectedOutcome||"Approved change")}</li>`).join("")}</ul></div><div class="job-actions"><button class="primary" data-execute>Approve & Execute</button><button class="secondary" data-revise>Revise Request</button><button class="secondary" data-close-job>Cancel</button></div>`;}
function completionPanel(result,error){if(error)return `<p class="error">${escapeHTML(error)}</p><div class="job-actions"><button class="primary" data-plan>Retry</button><button class="secondary" data-close-job>Close</button></div>`;const files=result?.execution?.filesWritten||[];return `<div class="deliverable"><strong>${escapeHTML(result?.summary||"Website retool completed and verified.")}</strong><p>${files.length} verified staging file${files.length===1?"":"s"} updated by the Kairos-native engine.</p></div><div class="job-actions"><button class="primary" data-new-job>Start Another Retool</button><button class="secondary" data-close-job>Close</button></div>`;}

function bind(){
  document.querySelectorAll("[data-center]").forEach(button=>button.onclick=()=>{state.activeCenter=button.dataset.center;resetPanels();render();setTimeout(()=>document.querySelector("#workspace")?.scrollIntoView({behavior:"smooth"}),20);});
  document.querySelector("[data-back]")?.addEventListener("click",()=>{state.activeCenter=null;resetPanels();render();});
  document.querySelectorAll("[data-child]").forEach(button=>button.onclick=()=>openChild(button.dataset.child));
  document.querySelector("[data-close-job]")?.addEventListener("click",()=>{state.job=null;state.plan=null;state.approval=null;render();});
  document.querySelector("[data-close-destination]")?.addEventListener("click",()=>{state.destination=null;state.destinationResult=null;state.destinationError=null;render();});
  document.querySelector("[data-run-destination]")?.addEventListener("click",runDestinationAction);
  document.querySelector("[data-new-destination]")?.addEventListener("click",()=>{state.destinationInput="";state.destinationResult=null;state.destinationError=null;render();});
  document.querySelector("[data-plan]")?.addEventListener("click",preparePlan);
  document.querySelector("[data-execute]")?.addEventListener("click",executePlan);
  document.querySelector("[data-revise]")?.addEventListener("click",()=>{state.job={mode:"input",objective:state.job.objective,action:"website"};state.plan=null;render();});
  document.querySelector("[data-new-job]")?.addEventListener("click",()=>{state.job={mode:"input",objective:"",action:"website"};state.plan=null;render();});
}
...
async function pollJob(submitted,type){const deadline=Date.now()+10*60*1000;while(Date.now()<deadline){await new Promise(resolve=>setTimeout(resolve,1000));const{response,body}=await fetchJSON(submitted.pollURL||`/api/shopify/staging/${type}/jobs/${submitted.jobID}`,{credentials:"include"});if(body?.status==="completed"&&body?.result)return body.result;if(["needs-attention","failed","cancelled"].includes(body?.status)||(!response.ok&&response.status!==202))throw new Error(body?.error?.message||body?.summary||`${type} did not complete.`);state.job.message=body?.summary||"Kairos is working…";render();}throw new Error("Kairos is still working, but this browser session reached its monitoring limit.");}
