const BUILD = "kairos-system-registry-ui-20260716-3";
...
function registryMarkup() {
  ...
  return `<section class="system-registry-summary"><article><span>Canonical actions</span><strong>${Number(registry.counts?.actions || 0)}</strong></article><article><span>Durable workflows</span><strong>${Number(registry.counts?.workflows || 0)}</strong></article><article><span>Work items</span><strong>${Number(registry.counts?.workItems || 0)}</strong></article><article><span>Execution receipts</span><strong>${Number(registry.counts?.executionReceipts || 0)}</strong></article><article><span>Verified native artifacts</span><strong>${Number(registry.counts?.nativeTaskArtifacts || 0)}</strong></article><article><span>Autonomy cycles</span><strong>${Number(registry.counts?.autonomyCycles || 0)}</strong></article></section><section class="system-registry-runtime"><div><span>Operational source</span><strong>${escapeHTML(registry.sourceOfTruth || "Unknown")}</strong></div><div><span>Enhanced account-scoped inference</span><strong data-state="${escapeAttribute(registry.intelligence?.enhancedRuntime)}">${escapeHTML(registry.intelligence?.provider || "unknown")}</strong><small>${escapeHTML([registry.intelligence?.model, registry.intelligence?.privacy].filter(Boolean).join(" · "))}</small></div><div><span>Self-hosted private inference</span><strong>${escapeHTML(registry.intelligence?.selfHostedPrivateRuntime || "not-configured")}</strong><small>${registry.intelligence?.selfHosted ? "MMG-controlled runtime selected" : "Optional upgrade; managed account-scoped inference is active"}</small></div><div><span>Autonomous operations</span><strong data-state="${escapeAttribute(registry.autonomy?.status)}">${escapeHTML(registry.autonomy?.status || "unknown")}</strong><small>${escapeHTML([registry.autonomy?.mode, registry.autonomy?.schedule].filter(Boolean).join(" · "))}</small></div><div><span>Native analysis</span><strong>${escapeHTML(registry.intelligence?.nativeObjectiveAnalysis || "unknown")}</strong></div><div><span>Native execution</span><strong>${escapeHTML(registry.intelligence?.nativeTaskExecution || "unknown")}</strong></div></section><label class="system-registry-search">Filter services, routes, owners, or actions<input type="search" value="${escapeAttribute(state.query)}" placeholder="Example: publishing, workflow, /api/offers" data-registry-query></label><div class="system-registry-table" role="table"><div class="system-registry-row system-registry-heading" role="row"><span>Action</span><span>Owner</span><span>Runtime</span><span>Persistence</span></div>${actions.map(actionRow).join("") || `<p class="system-registry-empty">No registry entries match this filter.</p>`}</div>`;
}
