const BUILD="kairos-social-production-ui-20260713-1";
...
function mount(){
  ...
  button.className="social-production-launch";
  button.textContent="Open Social Production";
  button.onclick=()=>{state.open=true;draw();};
...
function draw(){
  ...
  overlay.innerHTML=`<section class="social-production-panel"><header><div><p class="eyebrow">MMG Social Production</p><h2>Connector-Ready Content</h2><p>Build complete approval-ready social packages without publishing externally.</p></div><button data-close>×</button></header>${state.package?resultView():formView()}</section>`;
...
async function prepare(){
  ...
  await run("/api/social-production/prepare",{mode:state.mode,objective,audience,cta},body=>state.package=body.socialPackage);
}
